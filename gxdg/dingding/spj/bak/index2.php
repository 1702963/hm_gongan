...
function getkaoqin($_token,$workday){ //封装
...
<?php 
	$now_day=date('Y-m-d');
    $title_day=date("Y-m-d",strtotime("+10 day"));
?>
 
 
<table width="100%" cellpadding="0" cellspacing="0" class="zhiban">
  <tr>
    <td colspan="2" align="center"><h2><?php echo $now_day?> 至 <?php echo $title_day?> 值班安排</h2></td>
  </tr>
<?php 
	for($i=0;$i<=10;$i++){ 
...
?>  
		...
		<?php 
			if(!count($now_table[result][schedules])){
				echo "<tr><td width=24% align=center>".$i_day."</td>";
				echo "<td><span>无班次</span></td></tr>";
			} else {
				echo "<tr><td colspan=2 align=center bgcolor=#ebfeff><b>".$i_day."</b></td></tr>";
		...
		for($j=0;$j<count($banciarrs);){
         ...
         <tr class="tabhover">	
            ...
            <td align="center"><?php echo $banciarrs[$j][days]." 至 ".$banciarrs[$j+1][days]; ?></td>
         </tr>   
		<?php	$j=$j+2; 
		 	}	 ?>
