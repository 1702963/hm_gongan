...
//拼接参数,这个是JSAPI用的
     $timeStamp=time();
	 $signature=sign($_ticket['ticket'], "abcdef", $timeStamp,"http://39.104.77.131/gjoa/?showmenu=false");
 
        $config = array(
            'url' => "http://39.104.77.131/gjoa/?showmenu=false",
            'nonceStr' => "abcdef",
...
$data = array('workDate'=>$workday,'offset'=>0,'size'=>200);
...
<table width="100%" cellpadding="0" cellspacing="0" class="zhiban">
  <tr>
    <td colspan="2" align="center" style="display:none"><h2><?php //echo $now_day?>  <?php //echo $title_day?> 值班表</h2></td>
  </tr>
<?php
 	//$now_table2= json_decode(getkaoqin($_token,'2022-01-14'),true);
	//echo $_token;
	//print_r($now_table2); 
	...
	[586650134] => 节假日白班 8
               ...
               // print_r($tmps);
				...
				for($px=0;$px<count($banciarrs);){
					if($banciarrs[$px][bc]==542690558){
						$banciarrses[0]=$banciarrs[$px];
						$banciarrses[1]=$banciarrs[$px+1];
						}
					if($banciarrs[$px][bc]==565490035){
						$banciarrses[2]=$banciarrs[$px];
						$banciarrses[3]=$banciarrs[$px+1];
						}
					if($banciarrs[$px][bc]==458420375){
						$banciarrses[4]=$banciarrs[$px];
						$banciarrses[5]=$banciarrs[$px+1];
						}
					if($banciarrs[$px][bc]==534545623){
						$banciarrses[6]=$banciarrs[$px];
						$banciarrses[7]=$banciarrs[$px+1];
						}
					if($banciarrs[$px][bc]==586650134){
						$banciarrses[8]=$banciarrs[$px];
						$banciarrses[9]=$banciarrs[$px+1];
						}
					if($banciarrs[$px][bc]==540655138){
						$banciarrses[10]=$banciarrs[$px];
						$banciarrses[11]=$banciarrs[$px+1];
						}	
					if($banciarrs[$px][bc]==540535340){
						$banciarrses[12]=$banciarrs[$px];
						$banciarrses[13]=$banciarrs[$px+1];
						}																																					
					 $px=$px+2;	
					}
					ksort($banciarrses);
		 ...
		 if(in_array($banciarrs[$j][bc],$basebanci) && $banciarrs[$j][noshow]==0){
			if(strtotime($i_day)<=strtotime(date("Y-m-d"))){ 
			 ...
			 $thisdk=json_decode(getpbdk($_token,'251461380426229750',$banciarrs[$j][userid],$dk_day),true);
			   ...
			   $dkindex=0;//默认一个打卡记录的索引
				  if($banciarrs[$j][bc]==458420375){//值班员上午
					  $dkindex=0;
					  }
				  if($banciarrs[$j][bc]==534545623){//值班员下午
					  $dkindex=2;
					  }	
				  if($banciarrs[$j][bc]==540655138){//工作日晚班
					  $dkindex=2;
					  }	
				  if($banciarrs[$j][bc]==565490035){//带班晚班
					  $dkindex=2;
					  }						  				   
			   //==========================================================
			...
			if(is_array($reads)){
			   ...
			   if(in_array($banciarrs[$j][userid],$reads[send_result][read_user_id_list])){
				...
				if(in_array($banciarrs[$j][userid],$reads[send_result][forbidden_user_id_list])){
					$zt_str2="未达";
					}else{	
				$zt_str2="未读";
					 }
