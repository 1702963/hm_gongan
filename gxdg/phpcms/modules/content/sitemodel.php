...
class sitemodel extends admin
{
    ...
    public function init()
    {
        ...
        $datas = $this->db->listinfo("siteid={$this->siteid} AND type=0 AND modelid NOT IN (1,2,3)", '', $_GET['page'], 30);
        //模型文章数array('模型id'=>数量);
        $items = array();
    ...
    public function add()
    {
        if (isset($_POST['dosubmit'])) {
            $tname = $_POST['info']['tablename'];
            // 验证表名必须以 shangbao_ 开头
            if (strpos($tname, 'shangbao_') !== 0) {
                showmessage('模型表建名必须以 shangbao_ 开头', '?m=content&c=sitemodel&a=init');
            }
            ...
            showmessage(L('add_success'), '?m=content&c=sitemodel&a=init');
        } else {
