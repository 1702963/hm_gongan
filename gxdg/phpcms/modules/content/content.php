...
class content extends admin
{
    ...
    public function add()
    {
        if (isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
            ...
            if ($category['type'] == 0) {
                ...
                // 插入审批记录到 mj_content_approval
                if ($content_id) {
                    $model_cache = getcache('model', 'commons');
                    $table_name = $model_cache[$modelid]['tablename'];
                    $level1_user = isset($_POST['info']['level1_user']) ? intval($_POST['info']['level1_user']) : 0;
                    $level2_user = isset($_POST['info']['level2_user']) ? intval($_POST['info']['level2_user']) : 0;
                    $submit_user = isset($_SESSION['userid']) ? intval($_SESSION['userid']) : 0;
                    ...
                    $approval_db = pc_base::load_model('content_approval_model');
                    $approval_db->insert($approval_data);
                }
            ...
            if (isset($_GET['catid']) && $_GET['catid']) {
                ...
                if ($category['type'] == 0) {
                    ...
                    $admin_list = $admin_db->select('', 'userid, username, realname', '', 'userid ASC');
    ...
    public function edit()
    {
        ...
        if (isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
            ...
            $table_name = $model_cache[$modelid]['tablename'];
            $approval_db = pc_base::load_model('content_approval_model');
            $approval = $approval_db->get_one(array('table_name' => $table_name, 'record_id' => $id));

            // 处理审批操作
            $approval_action = isset($_POST['approval_action']) ? intval($_POST['approval_action']) : 0;
            ...
            $level2_name = '';
            $current_userid = intval($_SESSION['userid']);
            $is_level1_approver = false;
            $is_level2_approver = false;
            $show_level1_approval = false;
            $show_level2_approval = false;
            $level1_status = 0;
            $level2_status = 0;
            if ($approval) {
                ...
                $show_level2_approval = ($is_level2_approver && $level1_status == 1 && $level2_status == 0);
                // 获取审批员姓名
                $admin_db = pc_base::load_model('admin_model');
                if ($approval['level1_user'] > 0) {
                    $level1_admin = $admin_db->get_one(array('userid' => $approval['level1_user']), 'realname, username');
                    $level1_name = $level1_admin['realname'] ? $level1_admin['realname'] : $level1_admin['username'];
                }
                if ($approval['level2_user'] > 0) {
                    $level2_admin = $admin_db->get_one(array('userid' => $approval['level2_user']), 'realname, username');
                    $level2_name = $level2_admin['realname'] ? $level2_admin['realname'] : $level2_admin['username'];
                }
            }
    ...
    public function search_admin()
    {
        ...
        $list = $admin_db->select($where, 'userid, username, realname', '20', 'userid ASC');
