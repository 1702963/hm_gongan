...
class kaoqinshenpi extends admin {
	var $db;
	...
	public function init() { //主列表
		  ...
		  $msgs = $this->db->select(" yuan='考勤' ".$where,"*","","id desc");
			
		include $this->admin_tpl('kaoqin_main');
	}
	...
	public function bmuser() { //部门审核
		  ...
		  if($msgs['inids']!=""){
			 $where.=" id in(".$msgs['inids'].") " ;
			  }else{
			 $where.="  bmuser=".$_SESSION['userid'];  //bmok=0 and
			  }
	...
	public function kaoqinshenpi_do() { //审核操作
		  ...
		  if($_POST['dook']!=""){  //保存审批
			...
			if($_POST['ty']=='bmuser'){ 
			  ...
			  $msgin['yuan']='考勤';
			  $msgin['adddt']=time();
			  $msgin['msg']=$bumen[$bmids].$_POST['yue']."考勤表等待您审核";
