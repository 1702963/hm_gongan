...
class jixiaoshenpi extends admin {
	...
	public function init() { //主列表
		...
		$msgs = $this->db->listinfo(" yuan='绩效考评' ".$where,$order = 'id desc',$page, $pages = '20');
		$pages = $this->db->pages;		  
	...
	public function bmuser() { //部门审核
		  ...
		  if($msgs['isshow']==0){  
			//因部门领导以上角色是批量审批，此处需要进行阅读状态批量处理
			$plsh= $this->db->select("yuan='".$msgs['yuan']."' and showuser=".$msgs['showuser']." and doty='".$msgs['doty']."' and isshow=0","*","","id desc");
			foreach($plsh as $v){
				$doids[]=$v["id"];
				}
			if(isset($doids)){$ids= implode(",",$doids);}else{$ids=$id;}	

			$this->db->update(array('isshow'=>1,'readdt'=>time()),"id in($ids)");  
			  }
