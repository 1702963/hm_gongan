...
?>
<div class="cat_total">
	<div class="cat_total_con ra"><div class="tcon"><span><a href="javascript:;" onClick="gangwei()" style="color:#ffffff">绩效考评审核申请</a></span></div></div>
...
if(is_array($msgs)){
	...
	<tr onDblClick="show(<?=$info['id']?>);">
		...
		<td align="center" width="332"><?php if($info['dodt']==0){?><?php if($info['isshow']==1){echo "已读:".date("Y-m-d H:i:s",$info['readdt']);}else{echo "未读";}?><?php }else{ echo "已审批"; }?></td>
		<td align="center" width="309">
        <?php if($info['dodt']==0){?>
           <a href="index.php?m=shenpi&c=jixiaoshenpi&a=<?php echo $info['doty']?>&yues=<?php echo $info['yues']?>&msgid=<?php echo $info['id']?>&doty=<?php echo $info['doty']?>&bmid=<?php echo $info['addbm']?>" style="color:#F00">待审批</a>
        <?php }else{?>
           <a href="index.php?m=shenpi&c=jixiaoshenpi&a=<?php echo $info['doty']?>&yues=<?php echo $info['yues']?>&msgid=<?php echo $info['id']?>&doty=<?php echo $info['doty']?>&bmid=<?php echo $info['addbm']?>" style="color:#0F0">已审批</a>        
        <?php }?>
        </td>
	</tr>
	<?php
