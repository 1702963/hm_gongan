...
<div style="clear:both"></div>
    <div style=" width:100%;font-size:16px; text-align:center; margin-bottom:5px"><b>高新公安分局<?php $dys=$_GET['yues']."01"; echo date("Y年m月",strtotime($dys))?>辅警绩效奖金、加班费发放表</b>&nbsp; <input type="button" name="but1" value="审核绩效工资" style="border-radius:.4em;cursor:pointer; width:100px; height:25px; background-color:#06C; color:#FFF;display:" onclick="bmshenhe_do('<?php echo $_GET['yues']?>','<?php echo $_GET['doty']?>','<?php echo $show_table[0]['bmid']?>','<?php echo $_GET['msgid']?>')" <?php echo $buttoncan?>/></div>
...
<?php
	...
	<tr>		
		...
		<td align="center" ><?php echo $info['tsrenwu']?></a></td>
		<td align="center" ><?php echo $info['tcgongxian']?></a></td>
...
function bmshenhe_do(yue,doty,bmid,msgid){ 
		window.top.art.dialog({title:'绩效工资审核', id:'shows', iframe:'?m=shenpi&c=jxgongzishenpi&a=jxgongzishenpi_do&ty='+doty+'&yue='+yue+'&bmid='+bmid+'&msgid='+msgid,width:'480px',height:'200px'}, 	function(){var d = window.top.art.dialog({id:'shows'}).data.iframe;
