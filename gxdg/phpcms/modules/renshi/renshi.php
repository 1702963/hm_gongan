...
class renshi extends admin {
	var $db;
	public function __construct() {
		$this->db = pc_base::load_model('opinion2_model');
		//$this->db->set_db('gxdgdb');
	...
	public function init() {
		...
		/////////////////////////////////////
		///这里更新年龄，每次都触发，原则上应使用计划任务每天凌晨进行一次触发，暂时放在这里
		$this->db->table_name = 'v9_fujing';
		$rss = $this->db->select("shengri>=0","*");
		foreach($rss as $sr){
			$tmpage=date("Y")-date("Y",$sr['shengri']);
			$this->db->update("age=".$tmpage,"id=".$sr['id']);
			}		
		...
		//锁定表（单位级）
		if($_POST['dook']!=''){		
			if($_POST['lockclass']=="1"){
				$lockbm="dwid>0";
				}else{
					$lockbm="dwid=".intval($_POST['lockclass']);
					}
			 unset($_GET['doty']);
			 		
			 $uparr['islock']=intval($_POST['lockeds']);
			 $this->db->update($uparr,$lockbm);
			 //showmessage('锁表完成',HTTP_REFERER);		
			}
		
		 //单独锁定	
		 if($_GET['doty']!=""){
             $lockids=intval($_GET['ids']);
			 $uparr['islock']=intval($_GET['doty']);
 		
			 $this->db->update($uparr,array('id'=>$lockids));			  
			 }	
			 
		 //作废记录	
		 if($_GET['dofei']!=""){
             $feiids=intval($_GET['ids']);
			 $uparr['isok']=0;
 		
			 $this->db->update($uparr,array('id'=>$feiids));			  
			 }			 
	...
	if($_GET['dotongji']!=""){
		...
		if($_SESSION['roleid']>5){
		   $where .= " AND `dwid`  = ".$_SESSION['bmid'];
		}
		
		$page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
		$this->db->table_name = 'v9_fujing';
		$fujing = $this->db->listinfo($where,$order = $orders,$page, $pages = '12');
		$pages = $this->db->pages;
		...
		foreach($rss as $aaa){
		 ...
		 //统计当前各单位辅警数量
		$this->db->table_name = 'v9_fujing';
		$rss = $this->db->select("isok=1 and status=1",'count(id) as hj');
		$fjshuliang[1]=$rss[0]['hj'];
		
		$rss = $this->db->select("isok=1 and status=1 ",'count(dwid) as hj,dwid','','','dwid');	
		foreach($rss as $aaa){
			$fjshuliang[$aaa['dwid']]=$aaa['hj'];
			}							
		

		include $this->admin_tpl('fujing_list');
	}
	...
	function add()
	{
		...
		$this->db->insert($_POST['info']);
			...
			showmessage('添加成功','index.php?m=renshi&c=renshi&status=1');
		}
	...
	function addmj()
	{
		...
		include $this->admin_tpl('add_minjing');
	}	
	...
	function edit()
	{
		...
		if(isset($_POST['dosubmit'])) {
			...
			showmessage('操作完毕','index.php?m=renshi&c=renshi&status='.$_REQUEST['status']);
		}
		...
		if($fujing['ismj']==0){	
		  include $this->admin_tpl('edit_fujing');
			...
			include $this->admin_tpl('edit_mj');
			}
	}
	...
	function show()
	{
		...
		if($fujing['ismj']==0){	
		   include $this->admin_tpl('show_fujing');
			...
			include $this->admin_tpl('show_mj');
			}
	}
	...
	function lvli_mj()
	{
		...
		$this->db->table_name = 'v9_lvlib_mj';
		...
		include $this->admin_tpl('lvli_mj');
	}	
	...
	function techang_mj()
	{
		...
		$this->db->table_name = 'v9_techang_mj';
		...
		include $this->admin_tpl('techang_mj');
	}			
	///////////////////////////////////////////////////////////////////
	function jiaoyu()
	{
	...
	function jiaoyu_mj()
	{
		...
		$this->db->table_name = 'v9_peixun_mj';
		$peixun = $this->db->select(" fjid=$id and status>0",'*','','id asc');
		
		include $this->admin_tpl('jiaoyu_mj');
	}
	...
	{
		$id=$_GET['id'];
		$this->db->table_name = 'v9_jiashu_mj';
		//$jiashu = $this->db->select(" fjid=$id ",'*','','id asc');
		$page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
		$jiashu = $this->db->listinfo(" fjid=$id ",$order = 'id desc',$page, $pages = '3');
		$pages = $this->db->pages;

		include $this->admin_tpl('jiashu_mj');
	}
	
	function kaohe_mj()
	{
		$id=$_GET['id'];
		$this->db->table_name = 'v9_kaohe_mj';
		//$jiashu = $this->db->select(" fjid=$id ",'*','','id asc');
		$page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
		$peixun = $this->db->listinfo(" fjid=$id ",$order = 'id desc',$page, $pages = '3');
		$pages = $this->db->pages;

		include $this->admin_tpl('kaohe_mj');
	}
	...
	function addjiashu()
	{
		...
		$this->db->insert($_POST['info']);
			...
			showmessage('操作完毕','index.php?m=renshi&c=renshi&a=jiashu&id='.$_POST['id']);
		}
		
	}
	
	function addjiashu_mj()
	{
		if(isset($_POST['dosubmit'])) {
		...
		$this->db->insert($_POST['info']);
			...
			showmessage('操作完毕','index.php?m=renshi&c=renshi&a=jiashu_mj&id='.$_POST['id']);
		}
		
	}

	function addpeixun_mj()
	{
		if(isset($_POST['dosubmit'])) {
		...
		$this->db->table_name = 'v9_peixun_mj';
		    $_POST['info']['btime']=strtotime($_POST['info']['btime']);
			$_POST['info']['etime']=strtotime($_POST['info']['etime']);
			
		$this->db->insert($_POST['info']);
			
			
			showmessage('操作完毕','index.php?m=renshi&c=renshi&a=jiaoyu_mj&id='.$_POST['id']);
		}
		
	}

	function deletepeixun_mj()
	{
		$id = $_GET['id'];
		$this->db->table_name = 'v9_peixun_mj';
...
//////////////////////////////////////////////////////////////////////	
	function addbiaozhang_mj()
	{
		...
		$this->db->table_name = 'v9_biaozhang_mj';
		    $_POST['info']['bztime']=strtotime($_POST['info']['bztime']);

			
		$this->db->insert($_POST['info']);
			
			
			showmessage('操作完毕','index.php?m=renshi&c=renshi&a=biaozhang_mj&id='.$_POST['id']);
		}
		
	}

	function deletebiaozhang_mj()
	{
		$id = $_GET['id'];
		$this->db->table_name = 'v9_biaozhang_mj';
		if($this->db->delete(array('id'=>$_GET['id']))) {
	...
	function addchengjie_mj()
	{
		...
		$this->db->table_name = 'v9_chengjie_mj';
		    $_POST['info']['bztime']=strtotime($_POST['info']['bztime']);
		...
		$this->db->insert($_POST['info']);
			...
			showmessage('操作完毕','index.php?m=renshi&c=renshi&a=chengjie_mj&id='.$_POST['id']);
		}
		
	}

	function deletechengjie_mj()
	{
		$id = $_GET['id'];
		$this->db->table_name = 'v9_chengjie_mj';
		if($this->db->delete(array('id'=>$_GET['id']))) {
	...
	function addkaohe_mj()
	{
		...
		$this->db->insert($_POST['info']);
			...
			showmessage('操作完毕','index.php?m=renshi&c=renshi&a=kaohe_mj&id='.$_POST['id']);
		}
		
	}

	function deletekaohe_mj()
	{
		$id = $_GET['id'];
		$this->db->table_name = 'v9_kaohe_mj';
		if($this->db->delete(array('id'=>$_GET['id']))) {
...
///////////////////////////////////////////////////////////////////////
	...
	function biaozhang_mj()
	{
		$id=$_GET['id'];
		$this->db->table_name = 'v9_biaozhang_mj';
		$peixun = $this->db->select(" fjid=$id and status>0  ",'*','','id asc');
		
		include $this->admin_tpl('biaozhang_mj');
	}
	...
	function chengjie_mj()
	{
		...
		include $this->admin_tpl('chengjie_mj');
	}
