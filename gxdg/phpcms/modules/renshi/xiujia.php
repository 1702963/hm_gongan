...
class xiujia extends admin {
	...
	public function init() {
		...
		include $this->admin_tpl('xiujia_list');
	}
	...
	function addxiu()
	{
		...
		include $this->admin_tpl('xiujia_add');
	}
	...
	function shenhe()	{
		...
		$sq=$this->db->get_one("id=$id","*");
		$d0=strtotime($sq['qtime1']);
		$d2=strtotime($sq['qtime2']);
		if($d0<$d2){
			$q=$sq['qtime1'];
			$e=$sq['qtime2'];
			}else{
		...
		$this->db->table_name = 'v9_renshi_xiujialist';
		$ins['uid']=$sq['uid'];
		$ins['xid']=$sq['id'];
		$ins['leixing']=$sq['leixing'];
		$ins['dodt']=time();
		
		for($i=0;$i<=$days;$i++){
			$temp=strtotime($q . ' +'.$i.' day');
			$ins['days']=$ins['tjdt']=strtotime(date("Y-m-d",$temp));
			$ins['ns']=date("Y",$ins['days']);
			$ins['ms']=date("m",$ins['days']);
			$ins['ds']=date("d",$ins['days']);
		...
		$this->db->update($ups,array('id'=>$sq['id']));
	...
	function kqlist()
	{
		...
		include $this->admin_tpl('kq_list');
	}	

	function ddlist()
	{
		
		include $this->admin_tpl('dd_list');
	}
