...
include $this->admin_tpl('header_new', 'admin');
		...
		$where=" isok=1 ";
		$orders=" id desc ";
		$xjlist = $this->db->listinfo($where,$order = $orders,$page, $pages = '12');
		$pages = $this->db->pages;
?>
...
<div class="explain-col"> 
  快速工具:                       
<a href="index.php?m=renshi&c=xiujia&a=addxiu"><input type="button" value="休假申请" style="margin-left:10px; width:80px" class="doLock" name="dook"></a> &nbsp;&nbsp;
<a href="index.php?m=renshi&c=xiujia&a=kqlist"><input type="button" value="考勤情况" style="margin-left:10px; width:80px" class="doLock" name="dook"></a>
		</div>
...
<table width="100%" cellspacing="4" cellpadding="4" class="kotable">
	...
	<tr>
      ...
      <th width="204">单位</th>
	  <th width='109'>休假申请人</th>
	   <th width="175">起始时间</th>
      <th width="141">结束时间</th>
	  <th width="84">休假类型</th>
      <th width="242">申请理由</th>
      <th width="127">审批领导</th>
      <th width="231">申请时间</th>  
      <th width="174">申请状态</th>
      <th width="168">操作</th>
      </tr>
    </thead>
    <tbody>
...
if(is_array($xjlist)){
	...
	<tr>
	  ...
	  <td>
	  	<?php echo $info['qtime1']?>
      </td>
      <td><?php echo $info['qtime2']?></td>
	  <td><?php echo $info['leixing']?></td>
      <td><?php echo $info['shiyou']?></td>
      <td><?php echo $lingdao[$info['shuid']]?></td>
        ...
        <?php 
            if($info['shdt']==0){
				echo "待审";
			      ...
			      if($info["shenhe"]==0){
					 echo "<a title='审核时间:".date("Y-m-d H:i:s",$info['shdt'])."'><b>已拒绝</b></a>"; 
					  }else{
					echo "<a title='审核时间:".date("Y-m-d H:i:s",$info['shdt'])."'><b>已通过</b></a>"; 	  
						  }		
					}
		?>
      </td>
      <td>
        &nbsp;<a href="index.php?m=renshi&c=xiujia&a=shenhe&id=<?php echo $info['id']?>&status=<?php echo $status;?>" onclick="javascript:var r = confirm('确认审核申请吗？');if(!r){return false;}">审核</a>
        &nbsp;<a href="index.php?m=renshi&c=xiujia&a=dels&id=<?php echo $info['id']?>&status=<?php echo $status;?>" onclick="javascript:var r = confirm('确认删除记录吗？');if(!r){return false;}">删除</a>      
      </td>
      </tr>
   <?php
	$i++;}
