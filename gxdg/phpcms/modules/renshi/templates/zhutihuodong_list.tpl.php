...
.explain-col {border: 1px solid #3132a4;zoom: 1;background: #252682;padding: 8px 10px;line-height: 20px;color:#bbd8f1}
</style>
...
<div class="explain-col">
  快速工具:
  <a href="index.php?m=renshi&c=geren&a=addsx4_add"><input type="button" value="添加记录" style="margin-left:10px; width:80px" class="doLock" name="dook"></a>
</div>
...
$(document).ready(function() {
    ...
    $(".kotable tbody tr").mouseover(function() {$(this).addClass("iover");}).mouseout(function() {$(this).removeClass("iover");});
})
</script>
...
<tr>
  ...
  <td>
    <a href="index.php?m=renshi&c=geren&a=addsx4_edit&id=<?php echo $info['id']?>">编辑</a>
    <a href="index.php?m=renshi&c=geren&a=addsx4_del&id=<?php echo $info['id']?>" onclick="return confirm('确认删除该记录吗？');">删除</a>
  </td>
</tr>
<?php $i++;}}?>
</tbody>
        ...
        try {
            ...
            if (Array.isArray(images) && images.length > 0) {
                ...
                for (var i = 0; i < images.length; i++) {
                    html += '<div style="margin:10px;text-align:center;"><img src="' + images[i] + '" style="max-width:100%;border:1px solid #ddd;" /></div>';
                }
                html += '</div>';
                if (typeof art !== 'undefined' && art.dialog) {
                    art.dialog({title: '附件图片', content: html, lock: true});
                } else {
