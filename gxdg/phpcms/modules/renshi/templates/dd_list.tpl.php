...
&nbsp;
<a href="index.php?m=renshi&c=xiujia&a=ddlist"><input type="button" value="钉钉打卡记录" style="margin-left:10px; width:90px" class="doLock" name="dook"></a>
		</div>
...
$ym=date("t")-1;
//echo $ym;
$rli[]=strtotime(date("Y-m")."-01");
for($i=1;$i<=$ym;$i++){
	$temp=strtotime( date("Y-m-d",$rli[0]). ' +'.$i.' day');
	$rli[]=$temp;
	}

//print_r($rli);
 $kuan=intval((1675-310)/($ym+1));
...
<table width="100%" cellspacing="4" cellpadding="4" class="kotable">
	...
	<tr>
      ...
      <th width="130">姓名</th>
	  <th width='140'>DDid</th>
      <th width='140'>日期</th>
      <th width='140'>打卡类型</th>
      <th width='140'>打卡时间</th>
      <th width='140'>打卡情况</th>
      </tr>
    </thead>
    <tbody>
  <?php
...
if(is_array($fujing)){
	...
	<tr>
      ...
      <td><?php echo $info['ddid'];?></td>
      <td><?php echo date("Y-m-d",$info['wday2']);?></td>
      <td><?php if($info['dklx']=="OnDuty"){echo "上班打卡";}else{echo "下班打卡";}?></td>      
      <td><?php if($info['zt2']!="未打卡"){echo date("Y-m-d H:i:s",$info['dkday2']);}?></td>
      <td><?php echo $info['zt2'];?></td>
      </tr>
   <?php
	$i++;}
}
