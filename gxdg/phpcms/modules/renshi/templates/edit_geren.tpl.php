...
$ups["shenhe"]=$_POST['shenhe'];
$ups['beizhu']=$_POST['beizhu'];
$ups['shdt']=time();
$ups['shuser']=param::get_cookie('admin_username');

$this->db->update($ups,"id=".intval($_POST['id']));
...
showmessage('操作完成','index.php?m=renshi&c=geren&a=init');
	...
	$id=intval($_GET['id']); 
    $jichu = $this->db->get_one(" id=$id and isok=1 ",'*');
	
	$sxtyid=$jichu['sxty'];
	
	//业务字段
	$rss = $this->db->select(" spid=$id and isok=1 ",'*','','id asc');
	foreach($rss as $v){
	  $sx[$v['fname']]=$v['vals'];	
		}
?>
...
<form action="?m=renshi&c=geren&a=shenhe&sxty=<?php echo intval($_GET['sxty']) ?>" method="POST" name="myform" id="myform">
<input type="hidden" name="id" value="<?php echo $jichu['id']?>" />
...
<table cellpadding="0" cellspacing="0" class="baseinfo" align="center">
  <tr>
    <td width="100" align="right" class="infotitle"><span>姓名：</span><input type="hidden" name="info[uid]" id="uid" value="0" /></td>
    <td width="263"><input type="text" id="basexm" name="info[xingming]" value="<?php echo $jichu['xingming'];?>" class="infoinput" />
    <div id="inputlist">
        <ul>
        </ul>
    </div>    
    ...
    <td width="263"><input type="radio" class="rad" <?php if($jichu['sex']=='男'){?>checked<?php }?>  name="info[sex]"  id="nan" value="男"/>男
        <input type="radio" class="rad"   name="info[sex]" <?php if($jichu['sex']=='女'){?>checked<?php }?> value="女" id="nv"/>女</td>    
    <td width="100" align="right" class="infotitle">身份证号：</td>
    <td class="rb"><input type="text" name="info[sfz]" id="sfz" class="infoinput"  value="<?php echo $jichu['sfz'];?>"/>
      
    </td>
    </tr>
  <tr>
    <td align="right" class="infotitle">政治面貌：</td>
    <td width="263"><input type="text" name="info[zzmm]" id="zzmm" class="infoinput"  value="<?php echo $jichu['zzmm'];?>" /></td>
    <td width="100" align="right" class="infotitle">在职状态</td>
    <td colspan="3"><input type="radio" class="rad" <?php if($jichu['zaizhi']=='现职'){?>checked<?php }?>  name="info[zaizhi]" id="zzhi" value="现职"/>
      现职
        <input type="radio" class="rad"   name="info[zaizhi]" <?php if($jichu['zaizhi']=='退出现职尚未办理退休手续'){?>checked<?php }?> id="txiu" value="退出现职尚未办理退休手续"/>        
        退出现职尚未办理退休手续</td>
    </tr>
  <tr>
    <td align="right" class="infotitle">户籍地址：</td>
    <td width="263"><input type="text" name="info[hjdizhi]" id="hjdizhi" class="infoinput" value="<?php echo $jichu['hjdizhi'];?>" /></td>
    <td width="100" align="right" class="infotitle">民族：</td>
    <td width="263"><input type="text" name="info[minzu]" id="minzu" class="infoinput" value="<?php echo $jichu['minzu'];?>" /></td>
    <td width="100" align="right" class="infotitle">&nbsp;</td>
    <td class="rb">&nbsp;</td>
    </tr>
  <tr>
    <td align="right" class="infotitle">工作单位</td>
    <td width="263"><input name="info[danwei]" type="text" id="danwei" class="infoinput" value="<?php echo $jichu['danwei'];?>" /></td>
    <td width="100" align="right" class="infotitle">现任职务</td>
    <td width="263"><input name="info[zhiwu]" type="text" id="zhiwu" class="infoinput" value="<?php echo $jichu['zhiwu'];?>" /></td>
    <td width="100" align="right" class="infotitle">职级</td>
    <td class="rb"><input name="info[zhiji]" type="text" id="zhiji" class="infoinput" value="<?php echo $jichu['zhiji'];?>" /></td>
    </tr>
</table>
</div>
...
<table cellpadding="0" cellspacing="0" class="baseinfo">
  <tr>
    ...
    <td width="344"><?php echo form::date('shi[qianfa]',$sx['qianfa'],0,0,'false');?></td>    
    <td width="140" align="right" class="infotitle">有效期至：</td>
    <td width="434"><?php echo form::date('shi[youxiao]',$sx['youxiao'],0,0,'false');?></td>
    </tr>
  <tr>
...
<div class="clear"></div>
<div class="tabcon">
<div class="title" style="width:150px">审核</div>
<table width="1427" cellpadding="0" cellspacing="0" class="baseinfo">
  <tr>
    <td width="141" align="right" class="infotitle"><span>审核状态：</span></td>
    <td><input type="radio" class="rad" <?php if($jichu['shenhe']==1){?>checked<?php }?>  name="shenhe" value="1"/> 通过
      <input type="radio" class="rad" <?php if($jichu['shenhe']==0){?>checked<?php }?>  name="shenhe" value="0"/> 拒绝</td>
    <td width="95" align="right" class="infotitle">审核人</td>
    <td width="544"><?php echo $jichu['shuser'];?>  &nbsp;  <?php if($jichu['shdt']>0){echo date("Y-m-d H:i:s",$jichu['shdt']);}?></td>
    </tr>
  <tr>
    <td align="right" class="infotitle">原因：</td>
    <td colspan="3"><input type="text" id="beizhu" name="beizhu" value="<?php echo $jichu['beizhu'];?>" class="infoinput" style="width:90%"/></td>
  </tr>
</table>
</div>
<div class="clear"></div>
         ...
         /*
          ...
          window.baidu.sug = function(data){
            ...
            for(var i=0; i<x.data.length; i++){
              _html+="<li id="+x.data[i].id+" xm="+x.data[i].xingming+" sfz="+x.data[i].sfz+" sex="+x.data[i].sex+" zzmm="+x.data[i].zzmmname+" hj="+x.data[i].hjdizhi+" mz="+x.data[i].minzu+" dw="+x.data[i].danwei+" zw="+x.data[i].zhiwuname+" zj="+x.data[i].cengjiname+">"+x.data[i][0]+"["+x.data[i].sex+","+x.data[i].shenfen+"，"+x.data[i].danwei+"]</li>";
            }}
            $("#inputlist ul").html(_html);
            if(t.val() == ""){
              $("#inputlist").hide();
