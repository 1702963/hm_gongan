...
class jlpeizhi extends admin {
	...
	{	
	   ...
	   //插入快照
	   $this->db->table_name = 'v9_jingli_kz';
	   foreach($jl_rs as $j){
		  ...
		  if(count($tjarr)>0){
			...
			if($isnew['hj']<1){ //如果新建写入
			  ...
			  foreach($tjarr as $ins){
			    $ups['v9_ying']=$ins['v9_ying'];
				$ups['fj_ying']=$ins['fj_ying'];
				$ups['v9_shi']=$ins['v9_shi'];
				$ups['fj_shi']=$ins['fj_shi'];
				$ups['v9_cha']=$ins['v9_ying']-$ins['v9_shi'];
