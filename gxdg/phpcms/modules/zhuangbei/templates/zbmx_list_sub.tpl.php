...
function gettrees($dbobj,$pid=0,$spid=0,$dotime=0,&$outstr ){
	...
	if(isset($tmprs[0])){
		...
		<td align=\"center\" width=\"340\"><a href=\"index.php?m=zhuangbei&c=zbmx&a=add&pid=".$t['id']."\" style=\"display:none\">增加子类目</a> &nbsp;&nbsp; <a href=\"index.php?m=zhuangbei&c=zbmx&a=edit&id=".$t['id']."\" style=\"display:none\">修改</a>&nbsp;&nbsp;<a href=\"javascript:;\" onclick=\"att_delete(this,'".$info['id']."')\">删除</a></td>
	</tr>";
...
$piciarr = $this->db->select('isdel=0 and dotime>0 ','dotime','',"dotime asc");	
?>
<div class="subnav">
        <form action="?m=zhuangbei&c=zbmx&a=init" method="get">
        <input type="hidden" name="m" value="zhuangbei" />
        <input type="hidden" name="c" value="zbmx" />
        <input type="hidden" name="a" value="init" />
        <input type="hidden" name="doty" value="subs" />
    <div class="content-menu ib-a blue line-x">
        ...
        &nbsp;&nbsp; 请选择批次 <select name="pici">
                                <?php foreach($piciarr as $p){?>
                                <option value="<?php echo $p['dotime']?>"  <?php if($p['dotime']==$dotime){echo "selected";}?>><?php echo $p['dotime']?></option>
                                <?php }?>
                              </select>  &nbsp; <input type="submit"  name="dosubs" value="确认" style="width:90px" class="button" />                    
    </div>
             </form>
</div>    
<div class="pad-lr-10">
...
<?php
    if($_GET['pici']!=""){
		$thispici=intval($_GET['pici']);
		}else{
		$thispici=-1;	
			}
	$this->db->table_name = 'mj_beizhuang_zidian';
	$zbmx=$this->db->select(" pid=0 and dotime=$thispici",'*','','px asc');
if(is_array($zbmx)){
	...
	<tr>
		...
		<td align="center" width="124"><a href="index.php?m=zhuangbei&c=zbmx&a=add&pid=<?php echo $info['id']?>" style="display:none">增加子类目</a> &nbsp;&nbsp; <a href="index.php?m=zhuangbei&c=zbmx&a=edit&id=<?=$info['id']?>" style="display:none">修改</a>&nbsp;&nbsp;<a href="javascript:;" onclick="att_delete(this,'<?php echo $info['id']?>')"><?php echo L('delete')?></a></td>
	</tr>
	<?php
