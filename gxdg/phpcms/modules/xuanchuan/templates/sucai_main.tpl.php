...
$_benweek=date("w");
/// 本年
$tjrs = $this->db->get_one(" isok=1 and dbtype='素材' and ns=$_bennian ",'count(*) as hj');
if($tjrs['hj']>0){$tjrs['hj']+=1;}
$_ns= $tjrs['hj'];
/// 本月
$tjrs = $this->db->get_one(" isok=1 and dbtype='素材' and ms=$_benyue ",'count(*) as hj');
if($tjrs['hj']>0){$tjrs['hj']+=1;}
$_ms= $tjrs['hj'];
/// 本周
$tjrs = $this->db->get_one(" isok=1 and dbtype='素材' and ws=$_benweek ",'count(*) as hj');
if($tjrs['hj']>0){$tjrs['hj']+=1;}
$_ws= $tjrs['hj'];

//国家
$tjrs = $this->db->get_one(" isok=1 and dbtype='素材' and mtjibie='国家级' ",'count(*) as hj');
if($tjrs['hj']>0){$tjrs['hj']+=1;}
$_mt_g= $tjrs['hj'];
//省级
$tjrs = $this->db->get_one(" isok=1 and dbtype='素材' and mtjibie='省（部）级' ",'count(*) as hj');
if($tjrs['hj']>0){$tjrs['hj']+=1;}
$_mt_sb= $tjrs['hj'];
//市级
$tjrs = $this->db->get_one(" isok=1 and dbtype='素材' and mtjibie='市级' ",'count(*) as hj');
if($tjrs['hj']>0){$tjrs['hj']+=1;}
$_mt_shi= $tjrs['hj'];
//区县
$tjrs = $this->db->get_one(" isok=1 and dbtype='素材' and mtjibie='区级' ",'count(*) as hj');
if($tjrs['hj']>0){$tjrs['hj']+=1;}
$_mt_q= $tjrs['hj'];
//市局
        ...
        <div id="modRenshi">
            ...
            <div class="modItemContent">
            	...
            	<div class="DataItem" data-scroll-reveal="enter right">
                    ...
                    <a href="?m=xuanchuan&c=sucai&a=lists">
                    <div class="DataItems" data-scroll-reveal="wait .4s enter right">
                        <div class="DataItemName">全部素材</div>
