...
include $this->admin_tpl('header_new', 'admin');
		...
		$where=" isok=1 and dbtype='素材' ";
		$orders=" id desc ";
		$xclist = $this->db->listinfo($where,$order = $orders,$page, $pages = '12');
		$pages = $this->db->pages;
?>
...
<input type="hidden" value="sucai" name="c">
<input type="hidden" value="lists" name="a">
<input type="hidden" value="<?php echo $status;?>" name="status">
<div class="explain-col"> 
  快速工具:   素材库管理 &nbsp;  
  <a href="?m=xuanchuan&c=sucai&a=addsc"><input type="button" value="新增" class="button" name="dook"></a>                      
		</div>
</form>
...
<table width="100%" border="0">
  <tr>
    <td height="30">
  素材名称: <input type="text" value="<?php echo $tagname?>" class="input-text" size="6" name="tagname">&nbsp;&nbsp;
  上报部门：<select name="dwid">
                       <option value="">不限</option> 
                       <?php foreach($bms as $k=>$v){?>
                       <option value="<?php echo $k?>" <?php if($k==$dwid){?>selected="selected"<?php }?> ><?php echo $v?></option>
                       <?php }?>
                      </select>&nbsp;&nbsp;
  采用情况：<select name="caiyong">
                       <option value="">不限</option> 
...
<table width="100%" cellspacing="0" class="kotable">
	...
	<tr>
      <th width='80'>序号</th>
      <th width="315">素材标题</th>
      <th width="120">作者</th>
	  <th width='166'>上报人</th>
	  <th width="182">上报单位</th>
	  <th width="179">上报时间</th>
      <th width="187">素材类型</th>
      <th width="204">采用情况</th>
	  <th width="137">审核情况</th>
      <th width="282">操作</th>
	</tr>
    </thead>
    <tbody>
...
if(is_array($xclist)){
	...
	<tr>
      ...
      <td><?php echo $info['zuozhe']?></td>
      <td><?php echo $info['sbren']?></td>
	  <td><?php echo $info['zzdanwei']?></td>
	  <td><?php echo date("Y-m-d",$info['indt'])?></td>
      <td><?php echo $info['gjleixing']?></td>
      <td><?php echo $info['caiyong']?></td>
	  <td><?php 
	         $shzt="未审核";
	         if($info['shenhe']==1){
				 $shzt="已审核";
	  ...
	  ?></td>
      <td>
         <a href="index.php?m=xuanchuan&c=sucai&a=showsc&id=<?php echo $info['id']?>&status=<?php echo $status;?>">查看</a>
        
        &nbsp;<a href="index.php?m=xuanchuan&c=sucai&a=editsc&id=<?php echo $info['id']?>&status=<?php echo $status;?>">修改</a>
        
        &nbsp;<a href="index.php?m=xuanchuan&c=sucai&a=tozips&id=<?php echo $info['id']?>&status=<?php echo $status;?>" onclick="javascript:var r = confirm('导出素材执行负载大，请谨慎执行');if(!r){return false;}">导出素材</a>
        
        &nbsp;<a href="index.php?m=xuanchuan&c=sucai&a=zuofei&dofei=1&ids=<?php echo $info['id']?>&status=<?php echo $status;?>" onclick="javascript:var r = confirm('确认作废当前记录吗？');if(!r){return false;}">作废</a>    
      </td>
	</tr>
