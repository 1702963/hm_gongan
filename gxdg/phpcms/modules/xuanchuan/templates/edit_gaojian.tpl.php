...
$_POST['in']['tjdt']=$_tjdt;
//上报时间
$_POST['in']['sbdt']=strtotime($_POST['in']['sbdt']);
$_POST['in']['sbns']=date("Y",$_POST['in']['sbdt']);
$_POST['in']['sbms']=date("m",$_POST['in']['sbdt']);
$_POST['in']['sbws']=date("w",$_POST['in']['sbdt']);
//审核时间
$_POST['in']['shdt']=$_indt;
$_POST['in']['shns']=date("Y",$_POST['in']['shdt']);
$_POST['in']['shms']=date("m",$_POST['in']['shdt']);
$_POST['in']['shws']=date("w",$_POST['in']['shdt']);
//采用时间
if($_POST['in']['caiyong']!="未采纳"){
$_POST['in']['cydt']=strtotime($_POST['in']['cydt']);
$_POST['in']['cyns']=date("Y",$_POST['in']['cydt']);
$_POST['in']['cyms']=date("m",$_POST['in']['cydt']);
$_POST['in']['cyws']=date("w",$_POST['in']['cydt']);	
	}
///部门转译
$_POST['in']['zzdanwei']=$bms[$_POST['in']['zzdwid']]; //作者单位
$_POST['in']['xcdanwei']=$bms[$_POST['in']['xcdwid']]; //宣传单位
//
...
showmessage('操作完成','index.php?m=xuanchuan&c=gaojian&a=lists');	
}
...
$_POST['sh']['shdt']=time();
$_POST['sh']['shns']=date("Y",$_POST['sh']['shdt']);
$_POST['sh']['shms']=date("m",$_POST['sh']['shdt']);
$_POST['sh']['shws']=date("w",$_POST['sh']['shdt']);
$_POST['sh']['shbz']=$_POST['in']['shbz'];
$_POST['sh']['shuser']=param::get_cookie('admin_username');

//print_r($_POST['sh']);exit;	
$this->db->update($_POST['sh'],"id=".$_POST['bzid']);
showmessage('操作完成','index.php?m=xuanchuan&c=gaojian&a=lists');		
	}
	
$xc=$this->db->get_one("id=".intval($_GET['id']));	
...
<form action="?m=xuanchuan&c=gaojian&a=editgj" method="POST" name="myform" id="myform">
<input type="hidden" name="bzid" value="<?php echo $xc['id']?>" />
...
<div class="tabcon">
<div class="title" style="width:150px">稿件基本情况</div>
<table cellpadding="0" cellspacing="0" class="baseinfo" align="center">
  <tr>
    <td align="right" class="infotitle">稿件标题：</td>
    <td colspan="5"><input type="text" id="title" name="in[title]" value="<?php echo $xc['title'];?>" class="infoinput" style="width:80%"/></td>
    </tr>
  <tr>
    <td width="117" align="right" class="infotitle"><span>作者：</span></td>
    <td width="246"><input type="text" id="zuozhe" name="in[zuozhe]" value="<?php echo $xc['zuozhe'];?>" class="infoinput" />
        <div id="inputlist">
        <ul>
        </ul>
       </div><input type="hidden" name="in[zzuid]" id="zzuid" value="<?php echo $xc['zzid']?>" />     
    </td>
    <td width="100" align="right" class="infotitle">上报单位：</td>
    <td width="211"><select name="in[zzdwid]" class="infoselect">
                      <?php foreach($bms as $k=>$v){?>
                      <option value="<?php echo $k?>" <?php if($xc['zzdwid']==$k){?>selected<?php }?>><?php echo $v?></option>
                      <?php }?>
                    </select> </td>    
    <td width="152" align="right" class="infotitle">上报日期：</td>
    <td width="172" class="rb"><?php echo form::date('in[sbdt]',date("Y-m-d",$xc['sbdt']),0,0,'false')?></td>
    </tr>
  <tr>
    <td align="right" class="infotitle">宣传对象：</td>
    <td width="246">
    <input type="text" id="xcren" name="in[xcren]" value="<?php echo $xc['xcren'];?>" class="infoinput" />
        <div id="inputlist2">
        <ul>
        </ul>
       </div><input type="hidden" name="in[xcuid]" id="xcuid" value="<?php echo $xc['xcuid']?>" /> 
    </td>

    <td width="100" align="right" class="infotitle">宣传单位:</td>
    <td><select name="in[xcdwid]" class="infoselect">
                      <?php foreach($bms as $k=>$v){?>
                      <option value="<?php echo $k?>" <?php if($xc['xcdwid']==$k){?>selected<?php }?>><?php echo $v?></option>
                      <?php }?>
                    </select></td>
    <td align="right" class="infotitle">上报人：</td>
    <td>
<input type="text" id="sbren" name="in[sbren]" value="<?php echo $xc['sbren'];?>" class="infoinput" />
        <div id="inputlist3">
        <ul>
        </ul>
       </div><input type="hidden" name="in[sbuid]" id="sbuid" value="<?php echo $sc['sbuid']?>" />     
    </td>
    </tr>
  <tr>
    <td align="right" class="infotitle">稿件类型：</td>
    <td width="246"><select name="in[gjleixing]" class="infoselect">
        <?php foreach($tagsp[1] as $v){?>
         <option value="<?php echo $v['name']?>" <?php if($v['name']==$xc['gjleixing']){?>selected="selected"<?php }?> ><?php echo $v['name']?></option>
        <?php }?>
      </select>
</td> 
    <td width="100" align="right" class="infotitle">采用状态：</td>
    <td><select name="in[caiyong]" class="infoselect">
        <?php foreach($tagsp[2] as $v){?>
         <option value="<?php echo $v['name']?>" <?php if($v['name']==$xc['caiyong']){?>selected="selected"<?php }?> ><?php echo $v['name']?></option>
        <?php }?>
      </select></td>
    <td align="right" >&nbsp;</td>
    <td></td>
  ...
  <tr>
    <td align="right" class="infotitle">发表媒体：</td>
    <td width="246"><input type="text" name="in[meiti]" id="meiti" class="infoinput"  value="<?php echo $xc['meiti'];?>" style="width:90%"/></td>
    <td width="100" align="right" class="infotitle">媒体级别：</td>
    <td><select name="in[mtjibie]" class="infoselect">
        <option value="">无</option>
        <?php foreach($tagsp[3] as $v){?>
         <option value="<?php echo $v['name']?>" <?php if($v['name']==$xc['mtjibie']){?>selected="selected"<?php }?> ><?php echo $v['name']?></option>
        <?php }?>
      </select></td>
    <td align="right" class="infotitle"><span class="infotitle">发表时间：</span></td>
    <td><?php echo form::date('in[cydt]',date("Y-m-d",$xc['cydt']),0,0,'false')?></td>
  </tr>
  <tr>
    <td align="right" class="infotitle">说明：</td>
    <td colspan="5">上报稿件应在“稿件内容”内粘贴素材文字内容，同时稿件电子档文件及相关材料已附件方式上传</td>
    </tr>
  <tr>
    <td align="right" class="infotitle">稿件内容：</td>
    <td colspan="5">
...
['Source','-','Templates'],
  ...
  <tr>
    <td align="right" class="infotitle">稿件资料：</td>
  ...
  <tr>
    <td align="right" class="infotitle">已上传资料：</td>
    <td colspan="5" id="uplist-zmups">
<?php 
if($xc['zmup']!=""){
		 $zmuparr=json_decode($xc['zmup'],true);
...
<table cellpadding="0" cellspacing="0" class="baseinfo">
  <tr>
    ...
    <td><input type="text" name="in[shbz]" id="shbz" class="infoinput"  value="<?php echo $xc['shbz'];?>" style="width:300px"/> &nbsp;  <?php if($_GET['a']=="editgj"){?><input type="submit" class="doshenhe" name="doshenhe" value="审核" /><?php }?></td>
    </tr>
  <tr>
    <td align="right" class="infotitle">处理备注:</td>
    <td colspan="3"><div id='content_tip'></div><textarea name="in[clbz]" id="content2" boxid="content"><?php echo $xc['clbz']?></textarea><script type="text/javascript" src="statics/js/ckeditor/ckeditor.js"></script><script type="text/javascript">
...
['Source','-','Templates'],
  ...
  <tr>
    ...
    <td colspan="3" id="uplist-qtups">
<?php 
	  if($xc['qtup']!=""){
		 $qtuparr=json_decode($xc['qtup'],true);
...
function getbmi(){
	...
	///////////////////////////////////////////////////////////////
     $(function(){
        $("#zuozhe").bind("keyup click",function(){
          var t=$(this),_html="";
                $.ajax({
                  async:false,
        ...
        $(document).delegate("#inputlist ul li","click",function(){
		  $("#zzuid").val($(this).attr("id"))	
		  $("#zuozhe").val($(this).attr("xm"))		  		  
        })
