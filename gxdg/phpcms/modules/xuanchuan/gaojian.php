...
class gaojian extends admin {
	...
	public function init() {						
		...
		include $this->admin_tpl('gaojian_main');
	}
	
	public function lists() {						
			
		include $this->admin_tpl('gaojian_list');
	}	
	
	public function addgj() {						
			
		include $this->admin_tpl('add_gaojian');
	}	
	
	public function showgj() {						
			
		include $this->admin_tpl('edit_gaojian');
	}	
	...
	public function tozip() {						
		showmessage('正在抽取稿件数据，请耐心......');	
		//include $this->admin_tpl('edit_sucai');
	}			
	
//////////////////////////////////////////////////////////////////////////////////////////////////////
///复杂的导出过程
///因信创环境不支持中文文件名，为了方便附件较多时方便管理附件，需要提供一个目录页面进行说明
///导出流程为:读出导出数据，把附件复制到dozip下临时文件夹，临时文件夹用uid命名，结构为down/files/ 附件目录  down/indx.html 目录文件 
	public function tozips() {			
		$id = $_GET['id'];
		$this->db->table_name = 'mj_xuanchuan_sucai';
        ...
        //检查目录是否存在
		$myroot=$rs['zzuid'];
		if(!file_exists($zipsroot.$myroot)){
		 @mkdir($zipsroot.$myroot);	
		 @mkdir($zipsroot.$myroot."/down");	
		 @mkdir($zipsroot.$myroot."/down/files");
			}
		 
		 //把附件移动到临时目录，之前需要删除全部临时文件
          foreach (scandir($zipsroot.$myroot."/down/files") as $filename) {
            if ($filename == '.' || $filename == '..') {continue;}
            $filepath = $zipsroot.$myroot."/down/files/". $filename;
            unlink($filepath);
           }
		 
		///////取出目录模板////////
        $mlmb = $zipsroot."/outtmpl.php";
        $mbnr = file_get_contents($mlmb);
		//设置模板替换字段
		$mbvar=array('[zy_leixing]'=>'zy_leixing','[zy_title]'=>'zy_title','[zy_zuozhe]'=>'zy_zuozhe','[zy_zzdanwei]'=>'zy_zzdanwei','[zy_cont]'=>'zy_cont','[zy_fujian]'=>'zy_fujian');
		//处理模板字段
		$vars['zy_leixing']=$rs['dbtype'];
		$vars['zy_title']=$rs['title'];
		$vars['zy_zuozhe']=$rs['zuozhe'];
		$vars['zy_zzdanwei']=$rs['zzdanwei'];
		$vars['zy_cont']=$rs['cont'];
		//附件需要特殊处理，信创系统不支持中文文件名
		$fujians="";
		if($rs['zmup']=="[]"){
			$fujians="无附件";
			}else{
		      $zmuparr=json_decode($rs['zmup'],true);
		      foreach($zmuparr as $k=>$v){ 
			      $v['url']=str_replace('../','',$v['url']);
				  $tmpf[]='<li><a href="files/'.basename($v['url']).'">'.$v['name'].'</a></li>'; 	
				  //复制到临时文件夹
				  $cok=copy($v['url'], $zipsroot.$myroot."/down/files/".basename($v['url']));
				  //echo 	$zipsroot.$myroot."/down/files/".basename($v['url']);		
				}
				$fujians=implode($tmpf);
			}
		//print_r($tmpf);exit;		
		////////////		
		$vars['zy_fujian']=$fujians;
        //替换模板内容
		foreach($mbvar as $k=>$v){
			$mbnr=str_replace($k,$vars[$v],$mbnr);
			}
		
		 ////生成目录文件,默认覆盖，不必删除目录文件
         $mulu = $zipsroot.$myroot."/down/index.html";
         file_put_contents($mulu, $mbnr);
		 
		 ///下面创建压缩包 ####################################################
         $zip = new ZipArchive();
         $zipName = $zipsroot.$myroot.'/'.$rs['zzuid'].'-down.zip';

         if ($zip->open($zipName, ZipArchive::CREATE | ZipArchive::OVERWRITE) === true) {
            $zip->addFile($mulu, 'index.html'); //加入文件
			//特别郁闷ZipArchive不支持添加文件夹
           foreach (scandir($zipsroot.$myroot."/down/files") as $filename) {
             if ($filename == '.' || $filename == '..') {continue;}
             $filepath = $zipsroot.$myroot."/down/files/". $filename;
		 ...
		 //#####################################################################
		 //输出到浏览器
         $downzipname=$rs['zuozhe']."-资源导出.zip";
