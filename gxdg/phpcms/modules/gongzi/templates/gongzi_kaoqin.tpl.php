...
<table width="100%" cellspacing="0" >
    ...
    <form action="" method="post">
     ...
     <input type="hidden" name="a" value="kaoqin" />
     <input type="hidden" name="dos" value="addkaoqin" />
		<tr>	
			<th align="left">*未检出可用的月度考勤表，点击按钮立即创建 <input type="submit" name="chuangjiankaoqin" value="创建"  style="border-radius:.4em;cursor:pointer; width:80px; height:25px; background-color:#06C; color:#FFF" /> <?php echo $msgss ?></th>
		</tr>
    </form>    
	</thead>
</table>
...
<table width="100%" cellspacing="0" >
	<thead>
		<tr>	
			<th width='10%' align="center">考勤表名</th>
            <th width='10%' align="center">考勤月度</th>
			<th width='10%' align="center">创建日期</th>
            <th width='10%' align="center">实际出勤天数</th>
...
<tbody>
<?php
if(is_array($kq_tables)){
	$i=0;
	foreach($kq_tables as $info){
    ...
    <input type="hidden" name="id" value="<?php echo $info[id]?>" />
        ...
        <td align="center" ><input type="text" name="sjts" value="<?php echo $info['sjts']?>" style="width:35px"/>&nbsp;<input type="submit" value="更新"  name="sjtsgx" style="border-radius:.4em;cursor:pointer; width:80px; height:25px; background-color:#06C; color:#FFF" /></td>
        ...
        <td align="center" ><?php if($info['isfinish']<1){?>未转结<?php }else{?><a href="index.php?m=gongzi&c=gzgl&a=daokq&tbname=<?php echo $info['tname']?>&yue=<?php echo $info['yue']?>">导出</a><?php }?></td>
...
<div style="margin:9px auto 9px auto; color:#F00">
* 创建月考勤数据时默认的“实际出勤天数”为当月非周六周日的天数，可能和真实出勤天数不同，如对实际出勤天数有调整请在“转结”操作前进行操作;本设置影响工资计算，请谨慎操作
</div> 
...
function showtables(id) {
		window.top.art.dialog({title:'查看考勤表', id:'showskaoqin', iframe:'?m=gongzi&c=gzgl&a=showkaoqin&id='+id ,width:'1020px',height:'550px'},function(){var d = window.top.art.dialog({id:'showskaoqin'}).data.iframe;
...
function dolocked(id,locked){
    ...
    if (isok == true) {
     location.href="index.php?m=gongzi&c=gzgl&a=kaoqin&islocked="+id+"&locked="+locked+"&pc_hash=<?php echo $_SESSION['pc_hash']?>"
    }
}

function dofinished(id,finished){
	//if(finished==1){
	//	alert("当前考勤表已结转，不允许再次操作")
	//	return false;
	//	}else{
	errs="结转数据将引起工资表数据联动，且无法取消操作，确认转结吗？"
	var isok=confirm(errs);
    //if (isok == true) {
     location.href="index.php?m=gongzi&c=gzgl&a=kaoqin&isfinisheded="+id+"&pc_hash=<?php echo $_SESSION['pc_hash']?>"
    //}
	//}	
}	

function doaddnew(id,finished){

	errs="确定创建下月考勤表吗？"
	var isok=confirm(errs);
    if (isok == true) {
     location.href="index.php?m=gongzi&c=gzgl&a=kaoqin&addkaoqin="+id+"&pc_hash=<?php echo $_SESSION['pc_hash']?>"
    }
}		
</script>