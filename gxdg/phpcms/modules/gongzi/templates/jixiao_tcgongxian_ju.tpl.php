...
?>
<div>
  <input type="button" name="addsq" value="审批申请" style="border-radius:.4em;cursor:pointer; width:80px; height:25px; background-color:#06C; color:#FFF" onclick="shenheju('<?php echo $nowyue[0]['yue']?>')"/>
  &nbsp; 可发放金额 <input type="text" id="canyongje" value="0" style="width:70px"/> <input type="button" value="均分" onclick="junfens()" style="width:40px; display:none"/>
</div>
<?php }?>
<div class="table-list" style="scroll-x:auto">
    <?php $jqnian=substr($nowyue[0]['yue'],0,4);$jqyue=substr($nowyue[0]['yue'],4,2)?>
    <div style=" width:100%;font-size:16px; text-align:center; margin-bottom:5px"><b>高新公安分局<?php echo $jqnian?>年<?php echo $jqyue?>月辅警突出贡献奖审批表</b></div>
...
<?php 
	...
	<tr>
        ...
        <td <?php echo $bcolor?> align="center" ><input type="text" name="je" id="<?php echo $i?>" value="<?php echo $info['je'] ?>" style="width:60px;ime-mode:Disabled" onblur="infas('<?php echo $i?>','<?php echo $info['id']?>')" onkeyup="if(isNaN(this.value)){this.value='0.00'}"/> 元</td>
		<td align="left" ><textarea name="beizhu" style="width:90%" onblur="ajax_do_bz('<?php echo $info['id']?>',this.value)"/><?php echo $info['beizhu']?></textarea></td>
		<td align="center" >
         <select name="zzcok" onchange="ajax_do('<?php echo $info['id']?>',this.value)" id="zzcok<?php echo $info['id']?>" >
          <option value="1" <?php if($info['zzcok']==1){?>selected="selected"<?php }?>>同意</option>
          <option value="-1">延迟发放</option>
...
<?php 
 $i++;
	...
	<tr>
        <td colspan="7" align="right" style="padding-right:30px">合计：<span id="hj"></span> 元</td>	
		</tr>
</tbody>
</table>
