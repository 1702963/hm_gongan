...
<table width="100%" border="0" align="left">
 <thead>
  <tr>
    ...
    <?php if($_SESSION['roleid']>5){?><th align="center" width="5%"><input type="button" name="but1" value="申请部门审核" style="border-radius:.4em;cursor:pointer; width:100px; height:25px; background-color:#06C; color:#FFF;display:" onclick="bmshenhe('<?php echo $_GET['yue']?>','<?php echo $_SESSION['bmid']?>')" <?php echo $buttoncan?>/></th> <?php }?> 
      ...
      <div style=" width:100%;font-size:12px; text-align:center; margin-bottom:5px">选择部门
               <select name="bms">
				...
				foreach($bumen as $k=>$v){
		          $spbmok = $this->db->get_one(" bmok=0 and yue=".$_GET['yue']." and bmid=".$k,"count(id) as tj");

				  if($spbmok['tj']>0){
				   $nosp='style="color:red"';
				  }else{
					 $nosp=""; 
					  }
					?>
                <option value="<?php echo $k?>" <?php if(intval($_GET['bms'])==$k){?> selected="selected"<?php }?> <?php echo $nosp;?> ><?php echo $v?><?php echo $xxxx?></option>
                <?php }?>
               </select>
               <input type="submit" name="dosel" value="定位" style="border-radius:.4em;cursor:pointer; width:80px; height:25px; background-color:#06C; color:#FFF"/> *未通过部门审核的单位以红色显示
      </div>
     </form> 
	 <?php }?>
...
<table width="1600px" cellspacing="0" border="1" style="margin-right:10px">
	<thead>
		<tr>
            ...
            <th align="center" style="font-size:12px">纪律作风</th> 
            <th align="center" style="font-size:12px">工作实绩</th>
