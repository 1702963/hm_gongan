...
class jixiao extends admin {
   ...
   //特殊任务审核
	...
	public function jxgzb() {
		...
		//获取指定月考勤表人员
			$this->db->table_name = $this->db->db_tablepre.'gz'.$_GET['yue'];
			$fjids = $this->db->select("","userid,bmid,kqtj","","id asc");
			 ...
			 /*if(in_array($ids1[$i]['bmid'],$sqbz['arr'])){
			  ...
			  if($ids1[$i]['kqtj']!=''){
				...
				if($t_zs==0){ //没有病事假
				  ...
				  if($ids1[$i]['jxjj']=="0.00"){ //基本绩效为0,不扣,再扣成负数了
				   ...
				   $ids1[$i]['koufayy']=$tyy;
				    ...
				    $koufajishgu= round($ids1[$i]['jxjj']/30,2); //按平均30天计算 
				    if($t_zs<5){
				...
				if($cqarr[13]>0){//如果辞职，算法完全不一样，重置入库数组
				      $ids1[$i]['jxjj']=round($ids1[$i]['jxjj']/30,2)*$cqarr[1];
				      $ids1[$i]['koufa']=0;
