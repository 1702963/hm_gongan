...
class jlpeizhi extends admin {
	...
	public function init() {
		...
		include $this->admin_tpl('jlpz_list');
	}
	...
	function sets()
	{
		...
		include $this->admin_tpl('jlpz_sets');
	}
	...
	{	
		...
		$kz_ns=date("Y");
		$kz_ms=date("m");
		$kz_tjdt=strtotime(date("Y-m-d"));
		$kz_indt=time();
		/////
	   $this->db->table_name = 'mj_jlset';
	   $pz_rs=$this->db->select("",'*','','id asc'); //获取警力配置
	   ...
	   $jl_rs=$this->db->select("status=1",'*','','id asc');  //仅获取在职人员
       
	   //插入快照
	   $this->db->table_name = 'mj_jingli_kz';
	   foreach($jl_rs as $j){
		  $ins['xingming']=$j['xingming'];
		  $ins['sex']=$j['sex'];
		  $ins['sfz']=$j['sfz'];
		  $ins['jh']=$j['gzz'];
		  $ins['ddid']=$j['ddid'];
		  ...
		  $this->db->table_name = 'mj_jingli_kz';
		  foreach($pz_rs as $d){ //轮询单位
		    $kztj_mj=$this->db->get_one(" ismj=1 and tjdt=$kz_tjdt and dwid=".$d['bmid'],' count(*) as hj'); //民警
			$kztj_fj=$this->db->get_one(" ismj=0 and tjdt=$kz_tjdt and dwid=".$d['bmid'],' count(*) as hj'); //辅警
			$out['pid']=$d['pid'];
			$out['bmid']=$d['bmid'];
			$out['mj_ying']=$d['mj_ying'];
			$out['fj_ying']=$d['fj_ying'];
			$out['mj_shi']=$kztj_mj['hj'];
			$out['fj_shi']=$kztj_fj['hj'];
			$out['tjdt']=$kz_tjdt;
			$out['ns']=date("Y",$kz_tjdt);
			$out['ys']=date("m",$kz_tjdt);
			$out['mj_cha']=$out['mj_ying']-$out['mj_shi'];
			$out['fj_cha']=$out['fj_ying']-$out['fj_shi'];
			
			$tjarr[]=$out; unset($out);//销毁临时变量
			
		  }
		 
		  //统计结果回写
		  if(count($tjarr)>0){
		    $this->db->table_name = 'mj_jl_yue';
			
			//判断新建还是更新
			$isnew=$this->db->get_one("tjdt=$kz_tjdt",' count(*) as hj'); //民警
			if($isnew['hj']<1){ //如果新建写入
			  foreach($tjarr as $ins){
			    $this->db->insert($ins);
			  }
			}else{ //如果已有则更新
			  foreach($tjarr as $ins){
			    $ups['mj_ying']=$ins['mj_ying'];
				$ups['fj_ying']=$ins['fj_ying'];
				$ups['mj_shi']=$ins['mj_shi'];
				$ups['fj_shi']=$ins['fj_shi'];
				$ups['mj_cha']=$ins['mj_ying']-$ins['mj_shi'];
				$ups['fj_cha']=$ins['fj_ying']-$ins['fj_shi'];
								
			    $this->db->update($ups,array("tjdt"=>$kz_tjdt,"bmid"=>$ins['bmid']));
			  }				
			}
			
		  }
	
	    showmessage('完成警力统计','index.php?m=renshi&c=jlpeizhi&a=init');
		//include $this->admin_tpl('jlpz_sets');
	}	
