...
class biaozhang extends admin {
	...
	public function init() {
		...
		$biaozhang = $this->db->listinfo('',$order = 'id asc',$page, $pages = '12');
		$pages = $this->db->pages;
	
		
			
		include $this->admin_tpl('biaozhang_list');
	}
	...
	function add()
	{
		...
		$this->db->insert($_POST['info']);
			...
			showmessage('操作完毕','index.php?m=bzcj&c=biaozhang&a=init');
		}
		...
		include $this->admin_tpl('add_biaozhang');
	}
	...
	function sh()
	{
		...
		$biaozhang = $this->db->get_one("id=$id",'*');
		
		include $this->admin_tpl('sh_biaozhang');
	}
	function show()
	{
		$id=$_REQUEST['id'];
		...
		include $this->admin_tpl('show_biaozhang');
	}
	//更新排序
