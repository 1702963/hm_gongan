...
<input type="hidden" value="fengxian" name="c">
<input type="hidden" value="init" name="a">
<input type="hidden" value="<?php echo $status;?>" name="status">
<div class="explain-col"> 
  快速工具:  风险/隐患上报排查 &nbsp;  
  <a href="?m=youfu&c=fengxian&a=addfx">
...
<table width="100%" border="0">
  <tr>
    <td height="30">
  上报类型：<select name="leixing">
                       <option value="">不限</option> 
                       <?php foreach($tagsp[2] as $k=>$v){?>
...
<table width="100%" cellspacing="0" class="kotable">
	...
	<tr>
      <th width='106'>序号</th>
      <th width="274">上报类型</th>
	  <th width='396'>上报单位</th>
	  <th width='397'>上报情况</th>
	  <th width="250">上报时间</th>
      <th width="146">审核状态</th>
      <th width="138">处理状态</th>
      <th width="149">操作</th>
	</tr>
    </thead>
    <tbody>
...
if(is_array($bzlist)){
	...
	<tr>
      ...
      <td><?php echo $info['lx']?></td>
      <td align="left"><?php echo $info['danwei']?></td>
      <td align="left"><?php echo mb_substr($info['cont'],0,10);?></td>
	  <td><?php echo date("Y-m-d",$info['indt'])?></td>
      <td><?php if($info['shenhe']==1){echo "已审核";}
	  if($info['shenhe']==0){echo "未审核";}
	  if($info['shenhe']==-1){echo "退回";}?></td>
      <td><?php if($info['clzt']==1){echo "已处理";}else{echo "未处理";}?></td>
      <td>
        <a href="?m=youfu&c=fengxian&a=editfx&id=<?php echo $info['id']?>"><input type="button" value="编辑" class="button"></a>   
      </td>
	</tr>
