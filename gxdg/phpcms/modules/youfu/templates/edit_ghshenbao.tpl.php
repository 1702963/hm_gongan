...
include $this->admin_tpl('header_new','admin');
		...
		$rss = $this->db->select(" isok=1 ",'id,name,pid','','id asc');
		foreach($rss as $aaa){
			if($aaa['pid']==0){
...
$this->db->table_name = 'mj_youfu_gonghuisb';
...
showmessage('操作完成','index.php?m=youfu&c=gonghui&a=init');	
}
...
<form action="?m=youfu&c=gonghui&a=editgh" method="POST" name="myform" id="myform">
...
<table cellpadding="0" cellspacing="0" class="baseinfo" align="center">
  <tr>
    <td width="117" align="right" class="infotitle"><span>申报人：</span></td>
    <td width="246"><input type="text" id="xingming" name="in[xingming]" value="<?php echo $xc['xingming'];?>" class="infoinput" />
      <div id="inputlist">
        <ul>
          </ul>
        </div><input type="hidden" name="in[uid]" id="uid" value="<?php echo $xc['uid']?>" />     
      </td>
    <td width="100" align="right" class="infotitle">申报单位：</td>
    <td width="211"><select name="in[dwid]" class="infoselect">
      <?php foreach($bms as $k=>$v){?>
      <option value="<?php echo $k?>" <?php if($xc['dwid']==$k){?>selected<?php }?>><?php echo $v?></option>
      <?php }?>
      </select> </td>    
  ...
  <tr>
    <td align="right" class="infotitle">申报类型：</td>
    <td width="246">
      <select name="in[leixing]" class="infoselect">
        <?php foreach($tagsp[3] as $v){?>
        <option value="<?php echo $v['name']?>" <?php if($v['name']==$xc['leixing']){?>selected="selected"<?php }?> ><?php echo $v['name']?></option>
        <?php }?>
        </select></td> 
...
['Source','-','Templates'],
  ...
  <tr>
    <td align="right" class="infotitle">申报表：</td>
  ...
  <tr>
    <td align="right" class="infotitle">已上传申报表：</td>
...
<table cellpadding="0" cellspacing="0" class="baseinfo">
  <tr>
    ...
    <td width="167" align="right" class="infotitle">审核备注：</td>
    <td colspan="3"><input type="text" name="shbz" id="shbz" class="infoinput"  value="<?php echo $xc['shbz'];?>" style="width:300px"/> &nbsp;  <input type="submit" class="doshenhe" name="doshenhe" value="审核" /></td>
    </tr>
  <tr>
    <td align="right" class="infotitle">申报状态：</td>
    <td width="311"><select name="in[bjzt]" class="infoselect">
       <option value="0" <?php if($xc['bjzt']==0){?>selected="selected"<?php }?>>未办结</option>
       <option value="1" <?php if($xc['bjzt']==1){?>selected="selected"<?php }?>>已办结</option>
      </select></td>
    <td width="167" align="right" class="infotitle">办结时间：</td>
    <td width="311"><?php if($xc['bjdt']==0){$xc['bjdt']=time();} echo form::date('in[bjdt]',date("Y-m-d",$xc['bjdt']),0,0,'false')?></td>
    <td width="154" align="right" class="infotitle">发放金额：</td>
    <td width="372"><input type="text" name="in[sfje]" id="sfje" class="infoinput"  value="<?php echo $xc['sfje'];?>" style="width:300px"/></td>
  </tr>
  <tr>
    <td align="right" class="infotitle">处理备注:</td>
    <td colspan="5"><div id='content_tip'></div><textarea name="in[clbz]" id="content2" boxid="content"><?php echo $xc['clbz']?></textarea><script type="text/javascript" src="statics/js/ckeditor/ckeditor.js"></script><script type="text/javascript">
