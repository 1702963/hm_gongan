...
showmessage('操作完成','index.php?m=youfu&c=fengxian&a=init');	
}
...
<form action="?m=youfu&c=fengxian&a=editfx" method="POST" name="myform" id="myform">
...
<div class="tabcon">
<div class="title" style="width:150px">上报基本情况</div>
<table cellpadding="0" cellspacing="0" class="baseinfo" align="center">
  <tr>
    <td width="117" align="right" class="infotitle">上报类型：</td>
    <td width="246">      <select name="in[lx]" class="infoselect">
        <?php foreach($tagsp[2] as $v){?>
        <option value="<?php echo $v['name']?>" <?php if($v['name']==$xc['lx']){?>selected="selected"<?php }?> ><?php echo $v['name']?></option>
        <?php }?>
        </select>     
      </td>
    <td width="100" align="right" class="infotitle">上报报单位：</td>
...
<table cellpadding="0" cellspacing="0" class="baseinfo">
  <tr>
    ...
    <td><input type="text" name="shbz" id="shbz" class="infoinput"  value="<?php echo $xc['shbz'];?>" style="width:300px"/> &nbsp;  <input type="submit" class="doshenhe" name="doshenhe" value="审核" /></td>
    </tr>
  <tr>
...
['Source','-','Templates'],
  ...
  <tr>
    <td align="right" class="infotitle">处理状态：</td>
    <td> <select name="in[clzt]" class="infoselect">
       <option value="0" <?php if($xc['clzt']==0){?>selected="selected"<?php }?>>未处理</option>
       <option value="1" <?php if($xc['clzt']==1){?>selected="selected"<?php }?>>已处理</option>
      </select></td>
    <td align="right" class="infotitle">处理时间：</td>
    <td><?php if($xc['clzt']==1){$xc['cldt']=time(); }echo form::date('in[cldt]',date("Y-m-d"),0,0,'false')?></td>
  </tr>
  <tr>
    <td align="right" class="infotitle">相关附件：</td>
