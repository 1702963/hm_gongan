...
include $this->admin_tpl('header_new', 'admin');
		...
		$this->db->table_name = 'mj_youfu_tijian';
//////////////////////////////////////
if($_POST['donew']!=""){
$_indt=time();
$_ns=date("Y",$_indt);
...
<input type="hidden" value="tijian" name="c">
<input type="hidden" value="init" name="a">
<input type="hidden" value="<?php echo $status;?>" name="status">
<div class="explain-col"> 
  快速工具:   健康体检 &nbsp;  
  <a href="?m=youfu&c=tijian&a=upbg">
  <input type="button" value="导入体检报告" class="button" name="dook"></a>  &nbsp;&nbsp;  |&nbsp; &nbsp;  
  <input type="button" value="录入体检情况" class="button" name="dook" onclick="showadd()"></div>
</form>
...
<table width="100%" border="0">
  <tr>
    <td height="30">
  体检人: <input type="text" value="<?php echo $tagname?>" class="input-text" size="6" name="tagname">&nbsp;&nbsp;
  体检年度：<select name="tagsid">
                       <option value="">不限</option> 
                       <?php for($i=2024;$i<=2030;$i++){?>
                       <option value="<?php echo $i?>" <?php if($i==$dwid){?>selected="selected"<?php }?> ><?php echo $i?></option>
                       <?php }?>
                      </select>&nbsp;&nbsp;
&nbsp;
...
<table width="100%" cellspacing="0" class="kotable">
	...
	<tr>
      <th width='56'>序号</th>
      <th width="171">体检人</th>
	  <th width='147'>体检年度</th>
	  <th width='70'>年龄</th>
	  <th width='76'>性别</th>
	  <th width="214">所属单位</th>
	  <th width="157">高血压</th>
	  <th width="153">高血脂</th>
	  <th width="145">高血糖</th>
	  <th width="135">高尿酸</th>
	  <th width="126">心脏异常</th>
	  <th width="100">囊肿、结节</th>
	  <th width="167">异常项目</th>
	  <th width="127">操作</th>
	</tr>
    </thead>
    <tbody>
...
<input type="hidden" value="tijian" name="c">
<input type="hidden" value="init" name="a">   
<tr id="addin" style="display:none">
      
      <td>&nbsp;</td>
      <td><input type="text" name="in[xingming]" id="addxm" />      
      <div id="inputlist">
        <ul></ul>
        </div><input type="hidden" name="in[uid]" id="uid" value="<?php echo $xc['uid']?>" /> </td>
      <td align="left"><?php echo form::date('niandt',"2024-01-01",0,0,'false')?></td>
      <td align="left"><input name="in[nl]" id="nl" value="" style="width:50px"/></td>
      <td align="left"><input name="in[sex]" id="sex" value="" style="width:50px"/></td>
      <td align="left"><input name="in[danwei]" id="danwei" value="" /><input type="hidden" name="in[dwid]" id="dwid" value="" /></td>
      <td align="left"><input type="checkbox" name="in[xy]" value="1" /></td>
      <td align="left"><input type="checkbox" name="in[xz]" value="1" /></td>
      <td align="left"><input type="checkbox" name="in[xt]" value="1" /></td>
      <td align="left"><input type="checkbox" name="in[niaos]" value="1" /></td>
      <td align="left"><input type="checkbox" name="in[xinz]" value="1" /></td>
      <td align="left"><input type="checkbox" name="in[jj]" value="1" /></td>
      <td align="left"><input name="in[ycxm]" value="" style="width:50px"/></td>
      <td>
       <input type="submit" value="录入" class="button" name="donew">  
      </td>
	</tr>
</form>  
  <?php
...
<input type="hidden" value="init" name="a">         
	<tr>
      ...
      <td align="left"><?php echo form::date('up[ns]',date("Y-m-d",$info['tjdt']),0,0,'false')?></td>
      <td align="left"><input name="up[nl]" value="<?php echo $info['nl']?>" style="width:50px"/></td>
      <td align="left"><input name="up[sex]" value="<?php echo $info['sex']?>" style="width:50px"/></td>
      <td align="left"><select name="up[dwid]">
                        <?php foreach($bms as $k=>$v){?>
                         <option value="<?php echo $k?>" <?php if($k==$info['dwid']){?>selected="selected"<?php }?> ><?php echo $v?> </option>
                        <?php }?>
                       </select></td>
      <td align="left"><input type="checkbox" name="up[xy]" value="<?php echo intval($info['xy'])?>" <?php if($info['xy']=="1"){?>checked="checked"<?php }?>/></td>
      <td align="left"><input type="checkbox" name="up[xz]" value="<?php echo intval($info['xz'])?>" <?php if($info['xz']=="1"){?>checked="checked"<?php }?>/></td>
      <td align="left"><input type="checkbox" name="up[xt]" value="<?php echo intval($info['xt'])?>" <?php if($info['xt']=="1"){?>checked="checked"<?php }?>/></td>
      <td align="left"><input type="checkbox" name="up[niaos]" value="<?php echo intval($info['niaos'])?>" <?php if($info['niaos']=="1"){?>checked="checked"<?php }?>/></td>
      <td align="left"><input type="checkbox" name="up[xinz]" value="<?php echo intval($info['xinz'])?>" <?php if($info['xinz']=="1"){?>checked="checked"<?php }?>/></td>
      <td align="left"><input type="checkbox" name="up[jj]" value="<?php echo intval($info['jj'])?>" <?php if($info['jj']=="1"){?>checked="checked"<?php }?>/></td>
      <td align="left"><input name="up[ycxm]" value="<?php echo $info['ycxm']?>" style="width:50px"/></td>
      <td>
        <input type="submit" value="编辑" class="button"></a>   
      </td>
	</tr>
</form>
   <?php
