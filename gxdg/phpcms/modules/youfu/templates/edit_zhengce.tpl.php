...
showmessage('操作完成','index.php?m=youfu&c=zhengce&a=init');	
}
...
<form action="?m=youfu&c=zhengce&a=editzc" method="POST" name="myform" id="myform">
...
<div class="tabcon">
<div class="title" style="width:150px">优抚政策</div>
<table cellpadding="0" cellspacing="0" class="baseinfo" align="center">
  <tr>
    <td width="117" align="right" class="infotitle">政策标题：</td>
    <td colspan="5"><input type="text" id="title" name="in[title]" value="<?php echo $xc['title'];?>" class="infoinput" style="width:80%"/></td>
    </tr>
  <tr>
    <td align="right" class="infotitle">信息类型：</td>
    <td width="246">
      <select name="in[zcleixing]" class="infoselect">
        <?php foreach($tagsp[1] as $v){?>
        <option value="<?php echo $v['name']?>" <?php if($v['name']==$xc['zcleixing']){?>selected="selected"<?php }?> ><?php echo $v['name']?></option>
        <?php }?>
        </select></td> 
    <td width="100" align="right" class="infotitle">信息状态：</td>
    <td colspan="3">
    <select name="in[isok]" class="infoselect">
        <option value="0" <?php if($xc['isok']==0){?>selected="selected"<?php }?> >禁用</option>
        <option value="1" <?php if($xc['isok']==1){?>selected="selected"<?php }?> >启用</option>

        </select>    
    </td>
  </tr>
  <tr>
    <td align="right" class="infotitle">信息内容：</td>
    <td colspan="5">
...
['Source','-','Templates'],
  ...
  <tr>
    <td align="right" class="infotitle">信息附件：</td>
