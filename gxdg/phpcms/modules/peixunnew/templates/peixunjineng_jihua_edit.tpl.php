...
<form action="?m=peixunnew&c=peixunjineng_jihua&a=editsave" method="POST" name="myform" id="myform">
<input type="hidden" name="id" value="<?php echo $this->info['id']?>" />

<div class="tableContent">

<div class="tabcon">
<div class="title" style="width:auto;">编辑警务技能培训计划</div>
  ...
  <tr>
    ...
    <td colspan="5">
      <select name="info[pxly]" id="pxly"
              style="width:300px;height:28px;background:transparent;color:#fff;border:1px solid #ddd;margin-left:5px;">
        <option value="内部培训" <?php echo $this->info['pxly'] == '内部培训' ? 'selected' : ''?>>内部培训</option>
        <option value="外部培训" <?php echo $this->info['pxly'] == '外部培训' ? 'selected' : ''?>>外部培训</option>
      </select>
    </td>
  </tr>
