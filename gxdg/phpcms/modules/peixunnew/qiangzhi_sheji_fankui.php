...
class qiangzhi_sheji_fankui extends admin
{
    var $db;
    ...
    public function __construct()
    {
        ...
        $this->db->table_name = 'v9_qiangzhi_sheji_fankui';
    ...
    public function init()
    {
        ...
        // 获取射击计划列表（用于显示射击标题）
        $this->db->table_name = 'v9_qiangzhi_sheji_jihua';
        $jihua_list = $this->db->select('', 'id,title,pxly');
        $jihua_map = array();
        ...
        $this->list = $list;
        $this->pages = $pages;
        include $this->admin_tpl('qiangzhi_sheji_fankui_list');
    }
    ...
    public function add()
    {
        // 加载射击计划列表（只显示进行中的）
        $this->db->table_name = 'v9_qiangzhi_sheji_jihua';
        $jihua_list = $this->db->select("status = 1", 'id,title,bmid,pxly,btime,etime');
        $this->jihua_list = $jihua_list;
        ...
        $this->select_bumen = $select_bumen;
        include $this->admin_tpl('qiangzhi_sheji_fankui_add');
    }
    ...
    public function addsave()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if (!$jihua_id) {
                showmessage('请选择射击计划', HTTP_REFERER);
            }
            if (!$fjid || !$fjname) {
                showmessage('请选择参射人员', HTTP_REFERER);
            }
            ...
            if ($exists) {
                showmessage('该人员在此射击计划中已存在反馈记录', HTTP_REFERER);
            }
            ...
            if ($result) {
                showmessage('添加成功', '?m=peixunnew&c=qiangzhi_sheji_fankui&a=init');
            } else {
    ...
    public function edit()
    {
        ...
        // 加载射击计划列表
        $this->db->table_name = 'v9_qiangzhi_sheji_jihua';
        $jihua_list = $this->db->select('', 'id,title,bmid,pxly');
        $this->jihua_list = $jihua_list;
        ...
        $this->select_bumen = $select_bumen;
        $this->info = $info;
        include $this->admin_tpl('qiangzhi_sheji_fankui_edit');
    }
    ...
    public function editsave()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if ($result !== false) {
                showmessage('修改成功', '?m=peixunnew&c=qiangzhi_sheji_fankui&a=init');
            } else {
    ...
    public function del()
    {
        ...
        if ($result) {
            showmessage('删除成功', '?m=peixunnew&c=qiangzhi_sheji_fankui&a=init');
        } else {
    ...
    public function upload_image()
    {
        ...
        $upload_path = 'uploadfile/peixun/qiangzhi_sheji_zhengshu/' . date('Y/md') . '/';
