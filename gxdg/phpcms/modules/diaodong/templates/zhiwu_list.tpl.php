...
<table width="100%" cellspacing="0">
	<thead>
		<tr>
			...
			<th width='619' align="center">原职务</th>
			<th width='240' align="center">变更后职务</th>
...
if(is_array($infos)){
	...
	<tr>
		...
		<td align="center" width="619"><?=$zhiwu[$info['oldzw']]?></td>
		
		
		<td align="center" width="217"><?=$zhiwu[$info['newzw']]?></td>
...
function sh(id) {
	window.top.art.dialog({title:'岗位变更审批', id:'showme2', iframe:'?m=diaodong&c=diaodong&a=shzw&id='+id ,width:'500px',height:'400px'});
}
function showxiang(id) {
	window.top.art.dialog({title:'查看详情', id:'showme', iframe:'?m=diaodong&c=diaodong&a=showzw&id='+id ,width:'900px',height:'650px'});
}
function att_delete(obj,aid){
	 window.top.art.dialog({content:'确认删除吗?', fixed:true, style:'confirm', id:'att_delete'}, 
	function(){
	$.get('?m=diaodong&c=diaodong&a=deletezw&id='+aid+'&pc_hash=<?php echo $_SESSION['pc_hash']?>',function(data){
