...
<?php
if($_SESSION['roleid']==9 || $_SESSION['roleid']==1){
?>
<div class="cat_total">
	<div class="cat_total_con ra"><div class="tcon"><span><a href="javascript:;" onClick="gangwei()" style="color:#ffffff">辅警离职申请</a></span></div></div>
...
<table width="96%" cellspacing="0" align="center">
	<thead>
		<tr>
			...
			<th width='812' align="center">部门-岗位-岗位辅助
		    </th>
			<th width='183' align="center">申请时间</th>
			<th width='310' align="center">审批状态</th>
...
if(is_array($infos)){
  ...
  <tr>
    <td width="12%" align="left"><strong>离职前</strong></td>
    <td width="38%" align="center"><?=$bumens[$info['olddw']]?></td>
    <td width="26%" align="center"><?=$gangweis[$info['oldgangwei']]?></td>
    <td width="24%" align="center"><?=$fzgangweis[$info['oldgangweifz']]?></td>
  </tr>
</table>

</td>
		
		<td align="center" width="183"><?php echo date("Y-m-d",$info['inputtime']);?></td>
...
function gangwei(id) {
		window.top.art.dialog({title:'辞职申请', id:'showme', iframe:'?m=shenpi&c=cizhishenpi&a=gwbd' ,width:'980px',height:'610px'});
	}
function bmsh(id) {
		window.top.art.dialog({title:'部门领导审核', id:'showmebm', iframe:'?m=shenpi&c=cizhishenpi&a=bmsh&id='+id ,width:'600px',height:'300px'});
	}
function fgsh(id) {
		window.top.art.dialog({title:'分管领导审核', id:'showmefg', iframe:'?m=shenpi&c=cizhishenpi&a=fgsh&id='+id ,width:'600px',height:'300px'});
	}
function zrsh(id) {
		window.top.art.dialog({title:'政治处主任审核', id:'showmezr', iframe:'?m=shenpi&c=cizhishenpi&a=zrsh&id='+id ,width:'600px',height:'300px'});
	}	
	function ldsh(id) {
		window.top.art.dialog({title:'局领导审核', id:'showmeld', iframe:'?m=shenpi&c=cizhishenpi&a=ldsh&id='+id ,width:'600px',height:'300px'});
	}
function show(id) {
		window.top.art.dialog({title:'详细信息', id:'showmeld', iframe:'?m=shenpi&c=cizhishenpi&a=show&id='+id ,width:'900px',height:'800px'});
	}	
function att_delete(obj,aid){
	 window.top.art.dialog({content:'确认删除吗?', fixed:true, style:'confirm', id:'att_delete'}, 
	function(){
	$.get('?m=shenpi&c=cizhishenpi&a=delete&id='+aid+'&pc_hash=<?php echo $_SESSION['pc_hash']?>',function(data){
