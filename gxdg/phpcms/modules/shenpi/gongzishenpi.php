...
class gongzishenpi extends admin {
	...
	public function init() { //主列表
		  ...
		  $msgs = $this->db->select(" yuan='工资' ".$where,"*","","id desc");
	...
	public function bmuser() { //部门审核
		  ...
		  //取出表格列名	  
          $rsss=$this->db->get_fields('kq'.$dbnames);
		  //print_r($rsss);
		  foreach($rsss as $k=>$v){
			if(strtotime($k)){
				$rowname[]=$k;
		  ...
		  $this->db->table_name = 'mj_kq'.$dbnames;
		...
		include $this->admin_tpl('kaoqin_shlist');
	}
	...
	public function zguser() { //主管领导审核
		  ...
		  $where.=" zguser=".$_SESSION['userid']." and bmid=".intval($_GET['bmid']); 
	...
	public function zzcuser() { //政治处主任审核
		  ...
		  $tables=$this->db->get_one("yue=$dbnames","*");
		  //print_r($tables);
		  ...
		  //取出表格列名	  
          //$rsss=$this->db->get_fields('gz'.$dbnames);
		  //print_r($rsss);
		  //foreach($rsss as $k=>$v){
			//	$rowname[]=$k;	
			//  }	  		  
		  ...
		  $this->db->table_name = 'mj_gz'.$dbnames;
		  ...
		  //获取各项合计
		   ...
		   foreach($rowname as $k=>$v){
			   if($k>3 && $k<14){
				  $hjs = $this->db->get_one($where,"sum(".$v[0].") as hj"); 
				  $hj[$k]=$hjs['hj']; 
				  }
			   }		  
		...
		include $this->admin_tpl('gongzi_shlist');
	}	

	public function zhengweiuser() { //政委审核
	...
	public function gongzishenpi_do() { //审核操作
		  ...
		  if($_POST['dook']!=""){  //保存审批
			...
			if($_POST['ty']=='bmuser'){ 
			  ...
			  $this->db->insert($msgin);				   
			   ...
			   //修改考勤状态	
			   $this->db->table_name = 'mj_kq'.$_POST['yue'];	
			   $this->db->update($uparr,array('bmid'=>$bmids));
			...
			if($_POST['ty']=='zzcuser'){ 
			  ...
			  if(intval($_POST['sq'])==0){
				  ...
				  $uparr['zzcdt']=time();
				  $uparr['zzcok']=1; 
				  $uparr['zhengweiuser']=intval($_POST['nextuser']); 
				}
			  ...
			  $msgin['yuan']='工资';
			  $msgin['adddt']=time();
			  $msgin['msg']=$_POST['yue']."工资表等待您审核";
			  $msgin['showuser']=intval($_POST['nextuser']);
			  $msgin['doty']='zhengweiuser';
			  $msgin['yues']=$_POST['yue'];
			  $msgin['adduser']=$_SESSION['userid'];
			  $msgin['addbm']=0;
			  $this->db->insert($msgin);				   
			...
			//政委
			if($_POST['ty']=='zhengweiuser'){ 
			  $bmids=intval($_POST['bmid']);
			  if(intval($_POST['nextuser'])==0){
			    showmessage("未匹配到审核角色,请联系管理员");
			  }			  
			  if(intval($_POST['sq'])==0){
				  $uparr['zhengweiuser']=0;//如果拒绝
				  $uparr['zhengweiok']=0;
				  }else{
				  $uparr['zhengweidt']=time();
				  $uparr['zhengweiok']=1; 
