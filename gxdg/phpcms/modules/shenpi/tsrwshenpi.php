...
class tsrwshenpi extends admin {
	var $db;
	...
	public function init() { //主列表
		  ...
		  $msgs = $this->db->select(" yuan='特殊任务' ".$where,"*","","id desc");
			
		include $this->admin_tpl('tsrw_main');
	}
	...
	public function bmuser() { //部门审核
		...
		//辅警
		  ...
		  $this->db->table_name = 'mj_tsrenwu';
			 ...
			 $where.=" bmok=0 and bmuser=".$_SESSION['userid']; 
		...
		include $this->admin_tpl('tsrw_shlist');
	}
	...
	public function zguser() { //主管领导审核
		...
		//辅警
		  ...
		  $where.=" fgok=0 and fguser=".$_SESSION['userid']; 
	...
	public function zzcuser() { //政治处主任审核
		...
		//辅警
		  ...
		  $where.=" zzcok=0 and zzcuser=".$_SESSION['userid'] ; 
	...
	public function juuser() { //局长审核
		...
		//辅警
		  ...
		  $where.=" juok=0 and juuser=".$_SESSION['userid']; 
	...
	public function tsrwshenpi_do() { //审核操作
		...
		//单位映射
		  ...
		  if($_POST['dook']!=""){  //保存审批
			...
			if($_POST['ty']=='bmuser'){ 
			  ...
			  $msgin['yuan']='特殊任务';
			  $msgin['adddt']=time();
			  $msgin['msg']=$bumen[$bmids].$_POST['yue']."特殊任务申请等待您审核";
			  ...
			  $this->db->insert($msgin);				   
			   ...
			   //修改状态	
			   $this->db->table_name = 'mj_tsrenwu';	
			   $this->db->update($uparr,array('bmok'=>0,'bmuser'=>$_SESSION['userid']));
			...
			if($_POST['ty']=='zguser'){ 
			  ...
			  //$this->db->insert($msgin);				   
			   ...
			   //修改考勤状态	
			   $this->db->table_name = 'mj_tsrenwu';	
			   $this->db->update($uparr,array('fgok'=>0,'fguser'=>$_SESSION['userid']));
			   showmessage("审核完成，请关闭窗口");
			...
			if($_POST['ty']=='juuser'){ 
			  ...
			  //$this->db->insert($msgin);				   
			   ...
			   //修改考勤状态	
			   $this->db->table_name = 'mj_tsrenwu';		
			   $this->db->update($uparr,array('juok'=>0,'juuser'=>$_SESSION['userid']));
		...
		include $this->admin_tpl('tsrw_shenqing_do');
	}	
