...
class member extends admin {
	...
	function add() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			// 保存所属部门
			$info['bumen_id'] = isset($_POST['info']['bumen_id']) ? intval($_POST['info']['bumen_id']) : 0;
			...
			// 通过mysqlhelper获取demo数据库的部门列表
			pc_base::load_sys_class('mysqlhelper', '', 0);
			$mysqlhelper = mysqlhelper::getInstance();
			$bumen_list = array();
			$bumen_data = $mysqlhelper->select('jj_bumen', 'id,name', '', 'listorder ASC, id ASC');
			if(is_array($bumen_data)) {
				foreach($bumen_data as $item) {
					$bumen_list[$item['id']] = $item['name'];
				}
			}
