...
<form action="?m=dangjian&c=zhibu&a=jingshijiaoyu_editsave" method="POST" name="myform" id="myform">
<input type="hidden" name="id" value="<?php echo $this->info['id']?>" />

<div class="tableContent">

<div class="tabcon">
<div class="title" style="width:auto;">开展警示教育情况</div>
<table cellpadding="0" cellspacing="0" class="baseinfo" align="center">
  ...
  <tr>
    ...
    <td colspan="5">
      <textarea name="info[content]" id="content"
                ...
                placeholder="请输入正文"><?php echo $this->info['content']?></textarea>
    </td>
  </tr>
  ...
  <tr>
    ...
    <td colspan="5">
      <input type="hidden" name="info[jingzhuang_photo]" id="jingzhuang_photo" value='<?php echo htmlspecialchars($this->info['jingzhuang_photo'])?>' />
...
// 初始化已有图片
$(function() {
    var existingPhoto = $('#jingzhuang_photo').val();
    if (existingPhoto) {
        try {
...
function uploadSingleFile(file) {
    ...
    $.ajax({
        url: 'index.php?m=dangjian&c=zhibu&a=upload_jingshijiaoyu_image',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
...
$(function(){
    ...
    // 初始化已选人员
    var existingRenyuan = $('#canhui_renyuan').val();
    if (existingRenyuan) {
