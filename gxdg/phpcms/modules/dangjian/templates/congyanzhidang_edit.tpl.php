...
<form action="?m=renshi&c=geren&a=addsx2_editsave" method="POST" name="myform" id="myform">
<input type="hidden" name="id" value="<?php echo $this->info['id']?>" />

<div class="tableContent">

<div class="tabcon">
<div class="title" style="width:auto;">落实全面从严治党工作</div>
<table cellpadding="0" cellspacing="0" class="baseinfo" align="center">
  ...
  <tr>
    ...
    <td colspan="5">
      <input type="date" name="info[content_time]" id="content_time" value="<?php echo $this->info['content_time_show']?>"
  ...
  <tr>
    ...
    <td colspan="5">
      <textarea name="info[neirong]" id="neirong"
                ...
                placeholder="请输入正文内容"><?php echo $this->info['neirong']?></textarea>
    </td>
  </tr>
...
$(function() {
    ...
    if (existingPeople) {
        selectedPeople = existingPeople.split(',');
...
$('#canhui_search').on('keyup', function() {
    ...
    searchTimer = setTimeout(function() {
        $.ajax({
            url: 'index.php?m=renshi&c=geren&a=search_fujing_congyanzhidang',
            type: 'GET',
            ...
            success: function(res) {
                if (res.status == 1 && res.data.length > 0) {
                    ...
                    for (var i = 0; i < res.data.length; i++) {
                        var item = res.data[i];
                        // 检查是否已选择
                        var isSelected = selectedPeople.indexOf(item.name) > -1;
                        if (!isSelected) {
                            var displayText = item.name + ' [' + (item.sex || '') + ', ' + (item.sfz || '') + ', ' + (item.danwei || '') + ']';
                            html += '<li data-name="' + item.name + '" style="padding:8px 12px;cursor:pointer;border-bottom:1px solid #444;">' + displayText + '</li>';
                        }
                    }
                    if (html) {
                        $('#canhui_inputlist ul').html(html);
...
$(document).delegate('#canhui_inputlist li', 'click', function() {
    ...
    if (selectedPeople.indexOf(name) === -1) {
...
function updateSelectedPeople() {
    ...
    for (var i = 0; i < selectedPeople.length; i++) {
        html += '<span style="display:inline-block;background:#3498db;color:#fff;padding:3px 8px;margin:2px;border-radius:3px;">'
              + selectedPeople[i]
              + ' <a href="javascript:void(0)" class="remove-person" data-name="' + selectedPeople[i] + '" style="color:#fff;text-decoration:none;margin-left:5px;">&times;</a>'
              + '</span>';
    }
...
$(function() {
    ...
    if (existingFujian) {
        try {
            fujianImages = JSON.parse(existingFujian);
            if (!Array.isArray(fujianImages)) {
                // 兼容旧数据：单个URL字符串转换为数组
                fujianImages = existingFujian ? [existingFujian] : [];
            }
        } catch (e) {
            // 解析失败，可能是旧格式的单个URL
            fujianImages = existingFujian ? [existingFujian] : [];
...
function uploadSingleFile(file) {
    ...
    $.ajax({
        url: 'index.php?m=renshi&c=geren&a=upload_congyanzhidang_image',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
...
function updateFujianPreview() {
    ...
    for (var i = 0; i < fujianImages.length; i++) {
        ...
        html += '<div style="display:inline-block;margin:5px;position:relative;">'
              ...
              + 'text-decoration:none;font-weight:bold;">&times;</a>'
              + '</div>';
    }
