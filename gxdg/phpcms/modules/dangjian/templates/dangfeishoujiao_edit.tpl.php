...
<form action="?m=dangjian&c=zhibu&a=dangfeishoujiao_editsave" method="POST" name="myform" id="myform">
...
<table cellpadding="0" cellspacing="0" class="baseinfo" align="center">
  ...
  <tr>
    ...
    <td colspan="5">
      <textarea name="info[zhengshou_qingkuang]" id="zhengshou_qingkuang"
                ...
                placeholder="请输入征收情况"><?php echo $this->info['zhengshou_qingkuang']?></textarea>
    </td>
  </tr>
  ...
  <tr>
    ...
    <td colspan="5">
      <input type="number" name="info[jiaona_jine]" id="jiaona_jine" value="<?php echo $this->info['jiaona_jine']?>" step="0.01" min="0"
  ...
  <tr>
    ...
    <td colspan="5">
      <input type="hidden" name="info[fujian_tupian]" id="fujian_tupian" value='<?php echo htmlspecialchars($this->info['fujian_tupian'])?>' />
  ...
  <tr>
    ...
    <td colspan="5">
      ...
      <input type="hidden" name="info[canyu_renyuan]" id="canyu_renyuan" value="<?php echo $this->info['canyu_renyuan']?>" />
...
// 初始化已有图片
$(function() {
    var existingPhoto = $('#fujian_tupian').val();
    if (existingPhoto) {
        try {
            photoImages = JSON.parse(existingPhoto);
            if (!Array.isArray(photoImages)) {
                // 兼容旧数据：单个URL字符串转换为数组
                photoImages = existingPhoto ? [existingPhoto] : [];
            }
        } catch (e) {
            // 解析失败，可能是旧格式的单个URL
            photoImages = existingPhoto ? [existingPhoto] : [];
...
$(function(){
    ...
    // 初始化已选人员
    var existingRenyuan = $('#canyu_renyuan').val();
    if (existingRenyuan) {
        selectedPeople = existingRenyuan.split(',').filter(function(name) {
            return name.trim() !== '';
        });
        updateSelectedList();
    }
