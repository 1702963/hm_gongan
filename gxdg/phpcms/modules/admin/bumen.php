...
class bumen extends admin {
	...
	function init () {
		...
		foreach($result as $r) {
			$r['cname'] = $r['name'];
			$r['str_manage'] = '<a class=modView href="?m=admin&c=bumen&a=add&parentid='.$r['id'].'&menuid='.$_GET['menuid'].'">添加子部门</a> | <a class=modChange href="?m=admin&c=bumen&a=edit&id='.$r['id'].'&menuid='.$_GET['menuid'].'">'.L('modify').'</a> | <a class=modCancel href="javascript:confirmurl(\'?m=admin&c=bumen&a=delete&id='.$r['id'].'&menuid='.$_GET['menuid'].'\',\''.L('confirm',array('message'=>$r['cname'])).'\')">'.L('delete').'</a> ';
			$array[] = $r;
		}
	...
	public function bumen_tree() {
        ...
        include $this->admin_tpl('bumen_tree');
		exit;
	}
	function add() {
