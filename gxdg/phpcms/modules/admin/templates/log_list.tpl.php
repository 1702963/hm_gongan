...
.doDel , .doSearch { border-radius:2px; border:0; cursor:pointer;color:#fff; height:20px; line-height:20px }
.doDel {background:#f60;width:120px;transition:all .2s ease-in-out}
.doDel:hover {background:#F90}
    ...
    <tbody>
		...
		<td>
        	<div class="explain-col">
				...
				<?php echo L('username')?>  <input type="text" value="" class="input-text" name="search[username]" size='10'>  
				<?php echo L('times')?>  <?php echo form::date('search[start_time]','','1')?> <?php echo L('to')?> <?php echo form::date('search[end_time]','','1')?>  
                <input type="submit" value="<?php echo L('determine_search')?>" class="doSearch" name="dosubmit"> &nbsp;&nbsp;&nbsp;&nbsp;
                <input type="button" class="doDel" name="del_log_4" value="<?php echo L('removed_data')?>" onclick="location='?m=admin&c=log&a=delete&week=4&menuid=<?php echo $_GET['menuid'];?>&pc_hash=<?php echo $_SESSION['pc_hash'];?>'"  />
		</div>
		</td>
		</tr>
    </tbody>
...
?>
    <tr> 
        <td align="center" style="color:#FFF"><?php echo $info['username']?></td>
        <td align="center" style="color:#FFF"><?php echo $info['module']?></td>
        <td align="left" title="<?php echo $info['querystring']?>" style="color:#FFF"><?php echo str_cut($info['querystring'], 40);?></td>
         <td align="center" style="color:#FFF"><?php echo $info['time'];//echo $info['lastusetime'] ? date('Y-m-d H:i', $info['lastusetime']):''?></td>
         <td align="center" style="color:#FFF"><?php echo $info['ip']?>　</td> 
    </tr>
<?php
	}
}
