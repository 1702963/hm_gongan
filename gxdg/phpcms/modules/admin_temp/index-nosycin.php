...
$this->db->table_name = 'yw_danwei';
	...
	$this->db->table_name = 'yw_gonggao';
	$gg=$this->db->get_one(" isdel=0 ",'*','listorder desc');	
	...
	public function public_main() {
		...
		//绑定事项名称
		$this->db = pc_base::load_model('opinion_model');
		$this->db->table_name = 'yw_shenpi';
		$rs = $this->db->select(' isdel=0 ','*','','id asc');
		$sp=array();
		...
		//绑定项目名称
		$this->db->table_name = 'yw_xiangmu';
		$rs2 = $this->db->select(' isdel=0 ','*','','id asc');
		$xm=array();
		
		foreach($rs2 as $bbb){
			$xm[$bbb['id']]=$bbb['xmname'];
			}	
