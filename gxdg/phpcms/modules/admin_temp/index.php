...
class index extends admin {
    ...
    $this->db->table_name = 'yw_danwei';
	$dw=$this->db->get_one("id=$dwid",'*');
	...
	public function login() {
		...
		if(isset($_GET['dosubmit'])) {
			//检查同步登录标记 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			if(isset($_GET['ukey'])){
				if(strlen(str_replace(" ","",$_GET['ukey']))==32){
					$tmpkey=str_replace(" ","",$_GET['ukey']);
				    //取出KEY
					$mykey=$this->db->get_one(array('ikey'=>$tmpkey));
					if($mykey){
						$is_slogin=1;
						$_POST['username']=$mykey['username'];
						}				
					}	
				}
			//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++			
            ...
            if($is_slogin!=1){
            //删除同步标记 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
			$this->db->update(array('ikey'=>'e000'),array('ikey'=>$tmpkey));
			}
						
			$_SESSION['userid'] = $r['userid'];
			$_SESSION['roleid'] = $r['roleid'];
			$_SESSION['dwid'] = $r['dwid'];
			$_SESSION['realname'] = $r['realname'];
			$_SESSION['zhiwu'] = $r['zhiwu'];
			$_SESSION['tel'] = $r['tel'];
	...
	if($_SESSION['roleid']==3||$_SESSION['roleid']==4||$_SESSION['roleid']==5||$_SESSION['roleid']==8||$_SESSION['roleid']==9||$_SESSION['roleid']==10){
		...
		if($_SESSION['roleid']==3){
			$gwurl="?m=xiangmu&c=xiangmu&a=fenfa";
			}
		if($_SESSION['roleid']==4){
			$where.=" and id not in(select xmid from yw_banli where roleid=4 and bltime<>0)";
			$gwurl="?m=xiangmu&c=xiangmu&a=banli";
			}
		if($_SESSION['roleid']==5){
			$where.=" and id  in(select xmid from yw_banli where roleid=5 and bltime=0)";
			$gwurl="?m=xiangmu&c=xiangmu&a=juzhang";
			}
		if($_SESSION['roleid']==10){
			$where.=" and id  in(select xmid from yw_banli where roleid=10 and bltime=0)";
			$gwurl="?m=xiangmu&c=xiangmu&a=juzhang";
			}	
		if($_SESSION['roleid']==8){
			$where.=" and id  in(select xmid from yw_banli where userid=".$_SESSION['userid']." and bltime=0)";
			$gwurl="?m=xiangmu&c=xiangmu&a=lingdao";
			}
		if($_SESSION['roleid']==9){
			$where.=" and id  in(select xmid from yw_banli where userid=".$_SESSION['userid']." and bltime=0)";
			$gwurl="?m=xiangmu&c=xiangmu&a=chushi";
			}	
			$where.=" order by id desc limit 5";		
		$rs=$this->db->query("select xmname,inputtime from yw_xiangmu where ".$where);
		$gw = $this->db->fetch_array($rs);
	
		
	}
	//请假处理/财务处理
	if($_SESSION['roleid']==5||$_SESSION['roleid']==8){
		
		if($_SESSION['roleid']==5){
			$wherej="  juzhangstatus=0 and lingdaostatus=1";
			$jurl="?m=jia&c=jia&a=init";
			$curl="?m=caiwu&c=caiwu&a=init";
			}
		if($_SESSION['roleid']==8){
			$wherej="  lingdaostatus=0 and lingdaoid=".$_SESSION['userid']."";
			$jurl="?m=jia&c=jia&a=init";
			$curl="?m=caiwu&c=caiwu&a=init";
			}
		$wherej.=" limit 5";	
						
		$rs=$this->db->query("select * from yw_jia where ".$wherej);
		//请假
		$js = $this->db->fetch_array($rs);
		$rs2=$this->db->query("select * from yw_caiwu where ".$wherej);
		//财务
		$cs = $this->db->fetch_array($rs2);
		
	}
	//日程
	    $rsr=$this->db->query("select * from yw_richeng where uid=".$_SESSION['userid']." limit 5");
		
		$ri = $this->db->fetch_array($rsr);
		//print_r($ri);
	//通知
	$rsg=$this->db->query("select * from yw_gonggao limit 5");
		
	$gg = $this->db->fetch_array($rsg);
