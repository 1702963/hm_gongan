...
$ccc=userBrowser();
if($ccc<>'Chrome'){
	echo "该系统不支持非Chrome内核,请切换到急速模式";exit;
	}
?>
...
.popfoot {width:100%;height:28px;position:absolute;z-index:888;bottom:0;display:block;text-align:right;line-height:28px;background:#014881;color:#fff}
.popfoot a {color:#fc0}
.winfoot {width:302px;height:203px;background:#f7f7f7;position:absolute;z-index:777;bottom:28px;display:block;right:-300px;background: url(statics/images/admin_img/zb/gongaobg.png) no-repeat;}
.winfoot-title {width:100%;height:36px;font:500 16px/36px Microsoft YaHei;color:#fff;text-indent:12px;cursor:pointer}
.winfoot-title span {float:left}
.winfoot-title em {float:right;margin-right:12px;font-size:14px}
.winfoot-c {width:270px;float:left;margin:15px;_display:inline;font:100 14px/24px Microsoft YaHei;color:#b4e3f3}
.winfoot-c a {color:#fff}
.winfoot-c a:hover {text-decoration:none;color:#fc0}
.win-r {width:14px;height:200px;bottom:28px;right:-40px;background:url(statics/images/admin_img/zb/winr.png) no-repeat;color:#fff;position:absolute;z-index:777;font-size:14px;padding:10px;font-family:Microsoft YaHei;cursor:pointer}
#gongao {right:-500px}
...
$(document).ready(function(){
	$("#gongao").stop().animate({ 'right': 0 },1000,function(){
			$("#quick").fadeIn(100);
			new Marquee("quick",2,1,470,56,70,0,0)
			}
		);
	$(".winfoot").stop().animate({'right':10},500);
	$(".winfoot-title").click(function(){
		$(".winfoot").animate({'right':-302},500,function(){
			$(".win-r").animate({'right':10},500);
			});
		});
	$(".win-r").click(function(){
...
<div class="header">
    ...
    <div class="col-auto">
    	<div class="log white cut_line"><?php echo L('hello'),$_SESSION['realname']?>  [<?php echo $dw['dwname']?>] [<?php echo $rolename?>]<span>|</span><a href="?m=admin&c=index&a=public_logout">[<?php echo L('exit')?>]</a><span>|</span><a href="javascript:;" onclick="lock_screen()"><img src="<?php echo IMG_PATH.'icon/lockscreen.png'?>"> <?php echo L('lockscreen')?></a>
    	</div>
...
function showxiang(id) {
	window.top.art.dialog({title:'查看详情', id:'showme', iframe:'?m=gonggao&c=gonggao&a=show&id='+id ,width:'900px',height:'650px'});
}
function wSize(){
	//这是一字符串
