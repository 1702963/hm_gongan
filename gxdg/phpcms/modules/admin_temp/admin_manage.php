...
class admin_manage extends admin {
	...
	public function init() {
		...
		$rs = $this->db->select("",'id,dwname','','id asc');
		$danwei=array();
		
		foreach($rs as $aaa){
			$danwei[$aaa['id']]=$aaa['dwname'];
			}
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			$admin_fields = array('username', 'email', 'password', 'encrypt','roleid','realname','dwid','zhiwu','tel');
	...
	public function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			$userid = $info['userid'];
			$admin_fields = array('username', 'email', 'roleid','realname','dwid','zhiwu','tel');
			foreach ($info as $k=>$value) {
				if (!in_array($k, $admin_fields)){
					unset($info[$k]);
				}
