...
<input type="hidden" value="yuqing" name="c">
<input type="hidden" value="init" name="a">
<input type="hidden" value="<?php echo $status;?>" name="status">
<div class="explain-col"> 
  快速工具:   舆情 &nbsp;                      
<a href="?m=jiandu&c=yuqing&a=addyq"><input type="button" value="新增" class="button" name="dook"></a>  
		</div>
</form>
...
<table width="100%" border="0">
  <tr>
    <td height="30">
  舆情关键字: <input type="text" value="<?php echo $xingming?>" class="input-text" size="6" name="xingming">&nbsp;&nbsp;
  舆情所涉单位：<select name="dwid">
...
<table width="100%" cellspacing="0" class="kotable">
	...
	<tr>
      ...
      <th width="149">舆情关键字</th>
	  <th width='127'>舆情所涉单位</th>
	  <th width="120">舆情采集时间</th>
      <th width="174">舆情描述</th>
	  <th width="185">复查情况</th>
      <th width="140">处理意见</th>  
      <th width="226">处理时间</th>
      <th width="152">处理结果</th>
      <th width="224">处理状态</th>
      <th width="276">操作</th>
	</tr>
    </thead>
    <tbody>
...
if(is_array($jdlist)){
	...
	<tr>
      ...
      <td><?php echo $i?></td>
      <td><?php echo $info['yqkey']?></td>
      <td><?php echo $info['yq_danwei']?></td>
	  <td><?php echo date("Y-m-d",$info['cjdt'])?></td>
      <td><?php echo mb_substr($info['wgnr'], 0, 10, "UTF-8")?></td>
	  <td><?php echo mb_substr($info['fcqk'], 0, 10, "UTF-8")?></td>
      <td><?php echo $info['clyj']?></td>
      <td><?php if($info['cled']==1){echo date("Y-m-d",$info['cldt']);}?></td>
      <td><?php echo mb_substr($info['cljg'], 0, 10, "UTF-8");?></td>
      <td><?php 
	          $clzt="违规信息采集";
	          if($info['clyj']!=""){
				 $clzt="已出具意见"; 
	  ...
	  ?></td>
      <td> 
        <a href="index.php?m=jiandu&c=yuqing&a=showyq&id=<?php echo $info['id']?>&status=<?php echo $status;?>">查看</a>
        
        &nbsp;<a href="index.php?m=jiandu&c=yuqing&a=edityq&id=<?php echo $info['id']?>&status=<?php echo $status;?>">修改</a>
        
        &nbsp;<a href="index.php?m=jiandu&c=yuqing&a=zuofei&dofei=1&ids=<?php echo $info['id']?>&status=<?php echo $status;?>" onclick="javascript:var r = confirm('确认作废当前记录吗？');if(!r){return false;}">作废</a>              
      </td>
	</tr>
   <?php
	$i++;}
