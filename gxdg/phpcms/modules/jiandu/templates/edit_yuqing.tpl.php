...
$this->db->table_name = 'mj_jiandu_yuqing';

if($_POST['dosubmit']!=""){

$_POST['in']['cjdt']=strtotime($_POST['in']['cjdt']);
$_POST['in']['cldt']=strtotime($_POST['in']['cldt']);
$_POST['in']['wgdt']=strtotime($_POST['in']['wgdt']);
...
showmessage('操作完成','index.php?m=jiandu&c=yuqing&a=init');	
...
showmessage('操作完成','index.php?m=jiandu&c=yuqing&a=init');		
 ...
 $jd=$this->db->get_one("id=".intval($_GET['id']));
...
<!--
	...
	parent.document.getElementById('center_frame').src = '?m=admin&c=bumen&a=bumen_tree&pc_hash=<?php echo $_SESSION['pc_hash'];?>&mm=jiandu&cc=yuqing&aa=init&status=<?php echo $status;?>';
...
<form action="?m=jiandu&c=yuqing&a=edityq" method="POST" name="myform" id="myform">
<input type="hidden" name="bzid" value="<?php echo $jd['id']?>" />
...
<div class="tabcon">
<div class="title" style="width:150px">舆情基本情况</div>
<table cellpadding="0" cellspacing="0" class="baseinfo" align="center">
  <tr>
    <td width="117" align="right" class="infotitle"><span>舆情关键字：</span></td>
    <td width="246"><input type="text" id="yqkey" name="in[yqkey]" value="<?php echo $jd['yqkey'];?>" class="infoinput" /></td>
    <td width="124" align="right" class="infotitle">舆情采集日期：</td>
    <td width="187"><?php echo form::date('in[cjdt]',date("Y-m-d",$jd['cjdt']),0,0,'false')?></td>    
    <td width="152" align="right" class="infotitle">舆情发生日期：</td>
    <td width="172" class="rb"><?php echo form::date('in[wgdt]',date("Y-m-d",$jd['wgdt']),0,0,'false')?></td>
    </tr>
  <tr>
    <td align="right" class="infotitle">舆情类型：</td>
    <td width="246">&nbsp;
      <select name="in[wgleixing]"  class="infoselect">
        <option value="执法态度" <?php if($jd['wgleixing']=="执法态度"){?>selected<?php }?>>执法态度</option>
        <option value="警务人员行为" <?php if($jd['wgleixing']=="警务人员行为"){?>selected<?php }?>>警务人员行为</option>
        <option value="警民关系" <?php if($jd['wgleixing']=="警民关系"){?>selected<?php }?>>警民关系</option>
        <option value="其他" <?php if($jd['wgleixing']=="其他"){?>selected<?php }?>>其他</option>
      </select></td>
    <td width="124" align="right" class="infotitle">舆情描述:</td>
    <td colspan="3"><span class="rb">
      <input type="text" name="in[wgnr]" id="wgnr" class="infoinput"  value="<?php echo $jd['wgnr'];?>" style="width:90%"/>
    </span></td>
    </tr>
  <tr>
    <td align="right" class="infotitle">舆情材料：</td>
    <td width="246"><input type="file" id="zmups" style="display:none" onchange="ufs('zmups')" name="zmup"/>&nbsp;<input type="button" value="上传" style="width:70px" onclick="onfiles('zmups')"/>&nbsp;
    <progress id="prob-zmups" value="0" max="100" style="display:none"></progress></td>
    <td width="124" align="right" >&nbsp;</td>
    <td colspan="3" id="msgs-zmups">&nbsp;</td>
  </tr>
  <tr>
...
<?php 
	  if($jd['zmup']!=""){
		 $zmuparr=json_decode($jd['zmup'],true);
...
<table cellpadding="0" cellspacing="0" class="baseinfo">
  <tr>
    <td align="right" class="infotitle">舆情涉及单位：</td>
    <td colspan="2"><span class="rb">
        <select name="in[yq_danwei]" class="infoselect">
                      <?php foreach($bms as $k=>$v){?>
                      <option value="<?php echo $v?>" <?php if($jd['yq_danwei']==$v){?>selected<?php }?>><?php echo $v?></option>
                      <?php }?>
                    </select>
    </span></td>
    </tr>
  <tr>
    <td align="right" class="infotitle">处理意见:</td>
    <td colspan="2"><div id='content_tip'></div><textarea name="in[clyj]" id="content" boxid="content"><?php echo $jd['clyj']?></textarea><script type="text/javascript" src="statics/js/ckeditor/ckeditor.js"></script><script type="text/javascript">
...
['Source','-','Templates'],
  ...
  <tr>
    ...
    <td colspan="2"><?php echo form::date('in[cldt]',date("Y-m-d",$jd['cldt']),0,0,'false')?> &nbsp;<input type="checkbox" name="in[cled]" value="1" <?php if($jd['cled']==1){?>checked="checked"<?php }?>/> &nbsp;已处理</td>
  </tr>
  <tr>
    <td align="right" class="infotitle">复查情况：</td>
    <td colspan="2"> <input type="text" name="in[fcqk]" id="fcqk" class="infoinput"  value="<?php echo $jd['fcqk'];?>" style="width:90%"/></td>
  </tr>
  <tr>
    <td width="118" align="right" class="infotitle">处理结果：</td>
    <td colspan="2"><div id='content_tip2'></div><textarea name="in[cljg]" id="content2" boxid="content"><?php echo $jd['cljg']?></textarea><script type="text/javascript" src="statics/js/ckeditor/ckeditor.js"></script><script type="text/javascript">
...
['Source','-','Templates'],
  ...
  <tr>
    ...
    <td width="362"><input type="file" name="qtup" id="qtups" style="display:none" onchange="ufs('qtups')"/>&nbsp;<input type="button" value="上传" style="width:70px" onclick="onfiles('qtups')"/>&nbsp;
    <progress id="prob-qtups" value="0" max="100" style="display:none"></progress></td>
    <td width="953" id="msgs-qtups">&nbsp;</td>
    </tr>
  <tr>
...
<?php 
	  if($jd['qtup']!=""){
		 $qtuparr=json_decode($jd['qtup'],true);
