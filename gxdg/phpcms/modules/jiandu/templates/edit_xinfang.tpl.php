...
$this->db->table_name = 'mj_jiandu_xinfang';

if($_POST['dosubmit']!=""){

$_POST['in']['xfdt']=strtotime($_POST['in']['xfdt']);
...
showmessage('操作完成','index.php?m=jiandu&c=xinfang&a=init');	
...
<!--
	...
	parent.document.getElementById('center_frame').src = '?m=admin&c=bumen&a=bumen_tree&pc_hash=<?php echo $_SESSION['pc_hash'];?>&mm=jiandu&cc=xinfang&aa=init&status=<?php echo $status;?>';
...
<form action="?m=jiandu&c=xinfang&a=editxf" method="POST" name="myform" id="myform">
...
<div class="tabcon">
<div class="title" style="width:150px">信访基本情况</div>
<table cellpadding="0" cellspacing="0" class="baseinfo" align="center">
  <tr>
    <td width="133" align="right" class="infotitle"><span>信访人：</span></td>
    <td width="230"><input type="text" id="xf_xingming" name="in[xf_xingming]" value="<?php echo $jd['xf_xingming'];?>" class="infoinput" /></td>
    <td width="124" align="right" class="infotitle">接访日期：</td>
    <td width="187"><?php echo form::date('in[xfdt]',date("Y-m-d",$jd['xfdt']),0,0,'false')?></td>    
    <td width="152" align="right" class="infotitle">信访来源：</td>
    <td width="172" class="rb"><select name="in[xfyuan]" id="xfyuan" class="infoselect">
      <option value="局长信箱" <?php if($jd['xfyuan']=="局长信箱"){?>selected<?php }?>>局长信箱</option>
      <option value="现场接访" <?php if($jd['xfyuan']=="现场接访"){?>selected<?php }?>>现场接访</option>
      <option value="12345市长热线" <?php if($jd['xfyuan']=="12345市长热线"){?>selected<?php }?>>12345市长热线</option>
      <option value="其他" <?php if($jd['xfyuan']=="其他"){?>selected<?php }?>>其他</option>
    </select></td>
    </tr>
  <tr>
    <td align="right" class="infotitle">信访人联系方式：</td>
    <td width="230">
      <input type="text" id="xflxfs" name="in[xflxfs]" value="<?php echo $jd['xflxfs'];?>" class="infoinput" /></td>
    <td width="124" align="right" class="infotitle">信访内容：:</td>
    <td colspan="3"><span class="rb">
      <input type="text" name="in[wgnr]" id="wgnr" class="infoinput"  value="<?php echo $jd['wgnr'];?>" style="width:90%"/>
    </span></td>
    </tr>
  <tr>
    <td align="right" class="infotitle">信访材料：</td>
...
<table cellpadding="0" cellspacing="0" class="baseinfo">
  <tr>
    <td align="right" class="infotitle">接访人：</td>
    <td colspan="2">
      <input type="text" name="in[xingming]" id="xingming" class="infoinput"  value="<?php echo $jd['xingming'];?>" />
        <div id="inputlist">
        <ul>
        </ul>
       </div>
       <input type="hidden" name="in[uid]" id="uid" value="<?php echo $jd['uid'];?>" />     
    </td>
    </tr>
  <tr>
    <td align="right" class="infotitle">信访涉及单位：</td>
    <td colspan="2"><span class="rb">
        <select name="in[jf_danwei]" class="infoselect">
                      <?php foreach($bms as $k=>$v){?>
                      <option value="<?php echo $v?>" <?php if($jd['jf_danwei']==$v){?>selected<?php }?>><?php echo $v?></option>
                      <?php }?>
                    </select>
    </span></td>
  </tr>
...
['Source','-','Templates'],
  ...
  <tr>
    ...
    <td width="349"><input type="file" name="qtup" id="qtups" style="display:none" onchange="ufs('qtups')"/>&nbsp;<input type="button" value="上传" style="width:70px" onclick="onfiles('qtups')"/>&nbsp;
    <progress id="prob-qtups" value="0" max="100" style="display:none"></progress></td>
    <td width="966" id="msgs-qtups">&nbsp;</td>
    </tr>
  <tr>
