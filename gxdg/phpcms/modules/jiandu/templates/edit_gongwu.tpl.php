...
showmessage('操作完成','index.php?m=jiandu&c=gongwu&a=init');	
...
<form action="?m=jiandu&c=gongwu&a=editche" method="POST" name="myform" id="myform">
...
<div class="tabcon">
<div class="title" style="width:150px">违规基本情况</div>
<table cellpadding="0" cellspacing="0" class="baseinfo" align="center">
  <tr>
    <td width="117" align="right" class="infotitle"><span>用车人：</span></td>
    <td width="246"><input type="text" id="xingming" name="in[yc_xingming]" value="<?php echo $jd['yc_xingming'];?>" class="infoinput" />
        <div id="inputlist">
        <ul>
        </ul>
       </div><input type="hidden" name="in[yc_uid]" id="uid" value="<?php echo $jd['yc_uid'];?>"/>     
    </td>
    <td width="100" align="right" class="infotitle">用车单位：</td>
    <td width="211"><select name="in[yc_danwei]" class="infoselect">
                      <?php foreach($bms as $k=>$v){?>
                      <option value="<?php echo $v?>" <?php if($jd['yc_danwei']==$v){?>selected<?php }?>><?php echo $v?></option>
                      <?php }?>
                    </select> </td>    
    <td width="152" align="right" class="infotitle">用车日期：</td>
    <td width="172" class="rb"><?php echo form::date('in[wgdt]',date("Y-m-d"),0,0,'false')?></td>
    </tr>
  <tr>
    <td align="right" class="infotitle">违规类型：</td>
    <td width="246">
      <select name="in[wgleixing]" class="infoselect">
        <option value="未审批用车" <?php if($jd['wgleixing']=="未审批用车"){?>selected<?php }?>>未审批用车</option>
        <option value="公车私用" <?php if($jd['wgleixing']=="公车私用"){?>selected<?php }?>>公车私用</option>
        <option value="改变用车路线" <?php if($jd['wgleixing']=="改变用车路线"){?>selected<?php }?>>改变用车路线</option>
        <option value="其他" <?php if($jd['wgleixing']=="其他"){?>selected<?php }?>>其他</option>
      </select></td>
    <td width="100" align="right" class="infotitle">违规事实:</td>
    <td colspan="3"><span class="rb">
      <input type="text" name="in[wgnr]" id="wgnr" class="infoinput"  value="<?php echo $jd['wgnr'];?>" style="width:90%"/>
    </span></td>
    </tr>
  <tr>
    <td align="right" class="infotitle">证据材料：</td>
  ...
  <tr>
    ...
    <td colspan="5" id="uplist-zmups"><?php 
	  if($jd['zmup']!=""){
...
<table cellpadding="0" cellspacing="0" class="baseinfo">
  <tr>
    <td align="right" class="infotitle">处理方式：</td>
    <td colspan="2"><span class="rb">
      <input type="text" name="in[clfs]" id="clfs" class="infoinput"  value="<?php echo $jd['clfs'];?>" />
    </span></td>
    </tr>
  <tr>
...
['Source','-','Templates'],
  ...
  <tr>
    ...
    <td width="318"><input type="file" name="qtup" id="qtups" style="display:none" onchange="ufs('qtups')"/>&nbsp;<input type="button" value="上传" style="width:70px" onclick="onfiles('qtups')"/>&nbsp;
    <progress id="prob-qtups" value="0" max="100" style="display:none"></progress></td>
    <td width="997" id="msgs-qtups">&nbsp;</td>
    </tr>
  <tr>
