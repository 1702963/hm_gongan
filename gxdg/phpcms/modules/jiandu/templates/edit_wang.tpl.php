...
$this->db->table_name = 'mj_jiandu_wang';
...
showmessage('操作完成','index.php?m=jiandu&c=wang&a=init');	
...
<!--
	...
	parent.document.getElementById('center_frame').src = '?m=admin&c=bumen&a=bumen_tree&pc_hash=<?php echo $_SESSION['pc_hash'];?>&mm=jiandu&cc=wang&aa=init&status=<?php echo $status;?>';
...
<form action="?m=jiandu&c=wang&a=editwang" method="POST" name="myform" id="myform">
...
<div class="tabcon">
<div class="title" style="width:150px">违纪基本情况</div>
<table cellpadding="0" cellspacing="0" class="baseinfo" align="center">
  <tr>
    <td width="133" align="right" class="infotitle"><span>违纪人：</span></td>
    <td width="230"><input type="text" id="xingming" name="in[xingming]" value="<?php echo $jd['xingming'];?>" class="infoinput" />
        <div id="inputlist">
        <ul>
        </ul>
       </div>
    ...
    <td width="124" align="right" class="infotitle">采集日期：</td>
    <td width="187"><?php echo form::date('in[cjdt]',date("Y-m-d",$jd['cjdt']),0,0,'false')?></td>    
    <td width="104" align="right" class="infotitle">所属单位：</td>
    <td width="220" class="rb"><input type="text" id="danwei" name="in[danwei]" value="<?php echo $jd['danwei'];?>" class="infoinput" /></td>
    </tr>
  <tr>
    <td align="right" class="infotitle">违纪类型：</td>
    <td width="230"><span class="rb">
      <select name="info[wgleixing]" id="wgleixing" class="infoselect">
        <option value="违反工作纪律" <?php if($jd['wgleixing']=="违反工作纪律"){?>selected<?php }?>>违反工作纪律</option>
        <option value="其他" <?php if($jd['wgleixing']=="其他"){?>selected<?php }?>>其他</option>
      </select>
    </span></td>
    <td width="124" align="right" class="infotitle">违纪情况：:</td>
    <td colspan="3"><span class="rb">
      <input type="text" name="in[wgnr]" id="wgnr" class="infoinput"  value="<?php echo $jd['wgnr'];?>" style="width:90%"/>
    </span></td>
    </tr>
  <tr>
    <td align="right" class="infotitle">相关材料：</td>
...
<table cellpadding="0" cellspacing="0" class="baseinfo">
  <tr>
    <td align="right" class="infotitle">采集人：</td>
    <td colspan="2"><span class="rb">
      <input type="text" name="in[cj_xingming]" id="cj_xingming" class="infoinput"  value="<?php echo $jd['cj_xingming'];?>" />
        <div id="inputlist2">
        <ul>
        </ul>
       </div>
       <input type="hidden" name="in[cj_uid]" id="cj_uid" value="<?php echo $jd['cj_uid'];?>" />         
    </span></td>
    </tr>
  <tr>
...
['Source','-','Templates'],
  ...
  <tr>
    ...
    <td width="279"><input type="file" name="qtup" id="qtups" style="display:none" onchange="ufs('qtups')"/>&nbsp;<input type="button" value="上传" style="width:70px" onclick="onfiles('qtups')"/>&nbsp;
    <progress id="prob-qtups" value="0" max="100" style="display:none"></progress></td>
    <td width="1036" id="msgs-qtups">&nbsp;</td>
    </tr>
  <tr>
...
function getbmi(){
	...
	///////////////////////////////////////////////////////////////
     ...
     $(function(){
        $("#cj_xingming").bind("keyup click",function(){
          var t=$(this),_html="";
                $.ajax({
                  async:false,
                  url:'api/getuser.php?sel='+t.val(),
        ...
        $(document).delegate("#inputlist2 ul li","click",function(){
		  $("#cj_uid").val($(this).attr("id"))	
		  $("#cj_xingming").val($(this).attr("xm"))	
		 // if($(this).attr('sex')=="男"){
		//	  $("#nan").attr("checked","checked");
		//	  }else{
		//		$("#nv").attr("checked","checked");  
