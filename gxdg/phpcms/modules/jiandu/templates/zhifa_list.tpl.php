...
<input type="hidden" value="zhifa" name="c">
<input type="hidden" value="init" name="a">
<input type="hidden" value="<?php echo $status;?>" name="status">
<div class="explain-col"> 
  快速工具:   执法办案 &nbsp;                      
<a href="?m=jiandu&c=zhifa&a=addzf"><input type="button" value="新增" class="button" name="dook"></a>  
		</div>
</form>
...
<table width="100%" cellspacing="0" class="kotable">
	...
	<tr>
      ...
      <th width="149">案件标题</th>
	  <th width='127'>立案时间</th>
	  <th width="120">结案时间</th>
      <th width="174">涉案人</th>
	  <th width="185">涉案人数</th>
      <th width="140">处理情况</th>  
      <th width="226">处理日期</th>
      <th width="209">处分情况</th>
      <th width="277">是否问责</th>
      <th width="166">操作</th>
	</tr>
    </thead>
    <tbody>
...
if(is_array($jdlist)){
	...
	<tr>
      ...
      <td><?php if($info['ladt']>strtotime("2020-1-1")){echo date("Y-m-d",$info['ladt']);}?></td>
	  <td><?php echo date("Y-m-d",$info['jadt'])?></td>
      <td><?php echo $info['xingming']?></td>
	  <td><?php echo $info['renshu']?></td>
      <td><?php echo mb_substr($info['clqk'], 0, 10, "UTF-8");?></td>
      <td><?php if($info['cled']==1){echo date("Y-m-d",$info['cldt']);}?></td>
      <td><?php echo mb_substr($info['cfqk'], 0, 10, "UTF-8")?></td>
      <td><?php echo $info['wenze']?></td>
      <td>
        <a href="index.php?m=jiandu&c=zhifa&a=showzf&id=<?php echo $info['id']?>&status=<?php echo $status;?>">查看</a>
        
        &nbsp;<a href="index.php?m=jiandu&c=zhifa&a=editzf&id=<?php echo $info['id']?>&status=<?php echo $status;?>">修改</a>
        
        &nbsp;<a href="index.php?m=jiandu&c=zhifa&a=zuofei&dofei=1&ids=<?php echo $info['id']?>&status=<?php echo $status;?>" onclick="javascript:var r = confirm('确认作废当前记录吗？');if(!r){return false;}">作废</a>           
      </td>
	</tr>
   <?php
	$i++;}
